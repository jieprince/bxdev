...
function orderSelectedResponse(result)
{
  ...
  try
  {
	...
	if(result.pay_code=='balance') {
		...
		if(parseFloat(result.amount) > parseFloat(result.user_money)) {
			
			var err_msg = '余额不足，请充值或使用其他支付方式！';
			document.getElementById('ECS_SURPLUS_NOTICE').innerHTML = err_msg;
			alert(err_msg);
			var paymentArr = document.getElementsByName("payment"); 
			for(var i=0; i<paymentArr.length; i++) { 
				paymentArr[i].checked=false;  
			}
			document.getElementById('payment_balance').setAttribute('disabled','disabled');
			
			return;
		}else{
			document.getElementById('ECS_SURPLUS').value = result.amount;
 ...
 * 改变金币
 */
function changeServiceMoney(val)
{
  if (selectedServiceMoney == val)
  {
    return;
  }
  else
  {
	  selectedServiceMoney = val;
  }

  Ajax.call('flow.php?step=change_service_money', 'service_money=' + val, changeServiceMoneyResponse, 'GET', 'JSON');
}

/* *
 * 改变金币回调函数
 */
function changeServiceMoneyResponse(obj)
{
  if (obj.error)
  {
    try
    {
      document.getElementById("ECS_SERVICE_MONEY_NOTICE").innerHTML = obj.error;
      document.getElementById('ECS_SURPLUS').value = '0';
      document.getElementById('ECS_SURPLUS').focus();
    }
    catch (ex) { }
  ...
  else
  {
    try
    {
      document.getElementById("ECS_SERVICE_MONEY_NOTICE").innerHTML = '';
    }
    catch (ex) { }
    orderSelectedResponse(obj.content);
  }
...
function checkOrderForm(frm)
{
  ...
  // 检查用户输入的金币
  if (document.getElementById("ECS_SERVICE_MONEY"))
  {
    var surplus = document.getElementById("ECS_SERVICE_MONEY").value;
    var error   = Utils.trim(Ajax.call('flow.php?step=check_surplus', 'surplus=' + surplus, null, 'GET', 'TEXT', false));
    ...
    if (error)
    {
      try
      {
        document.getElementById("ECS_SERVICE_MONEY_NOTICE").innerHTML = error;
      }
      catch (ex)
      {
      }
