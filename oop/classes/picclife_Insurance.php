 ...
 class picclife_Insurance 
 {
    										 ...
    										 $age)//被保险人年龄的影响因子,也可能是范围
    {
    	...
    	while ($duty_row = $_SGLOBAL['db']->fetch_array($product_duty_query))
    	{
    		...
    		{
    			$wheresql .= " AND product_career_id='$product_gender_id' ";
    		}
    		if($product_age_id != -1)
    		{
    			$wheresql .= " AND product_age_id='$product_age_id' ";
    		}
    		if($product_period_id != -1)
    		{
    			$wheresql .= " AND product_period_id='$product_period_id' ";
    		}
    		$sql="SELECT product_duty_price_id,amount,premium FROM t_insurance_product_duty_price
		    		WHERE product_duty_id='$product_duty_id' ".$wheresql." ORDER BY view_order";
    ...
    {
    	global $_SGLOBAL;
    	$sql="SELECT * FROM t_insurance_picclife_provincecode";
		$query = $_SGLOBAL['db']->query($sql);
    ...
    public function get_city_list_by_province_code($province_code)
    {
    	...
    	$sql="SELECT * FROM t_insurance_picclife_citycode WHERE city_code like '$pa%'";
		$query = $_SGLOBAL['db']->query($sql);
		$arr_list_city = array();
    	while ($row = $_SGLOBAL['db']->fetch_array($query))
    	{
    ...
    public function get_county_list_by_city_code($city_code)
    {
    	...
    	$sql="SELECT * FROM t_insurance_picclife_countycode WHERE county_code like '$pa%'";
		$query = $_SGLOBAL['db']->query($sql);
    ...
    public function get_classification_list()
    {
    	...
    	$sql="SELECT DISTINCT name_classification FROM t_insurance_picclife_careercode";
		$query = $_SGLOBAL['db']->query($sql);
     ...
     //获取行业下拉列表
    public function get_inclassification_list($name_classification)
    {
    	global $_SGLOBAL;

    	$sql="SELECT DISTINCT name_inclassification FROM t_insurance_picclife_careercode WHERE name_classification='$name_classification'";
		$query = $_SGLOBAL['db']->query($sql);
		$arr_list_industry = array();
    	while ($row = $_SGLOBAL['db']->fetch_array($query))
    	{
    ...
    //通过行业获取当前行业下的职业列表
    public function get_career_list_by_inclassification($name_inclassification)
    {
    	global $_SGLOBAL;

    	$sql="SELECT name_subclassification,code FROM t_insurance_picclife_careercode WHERE name_inclassification='$name_inclassification'";
		$query = $_SGLOBAL['db']->query($sql);
    ...
    public function get_province_name_by_code($code)
    {
    	...
    	$sql="SELECT province_name FROM t_insurance_picclife_provincecode WHERE province_code='$code'";
		$query = $_SGLOBAL['db']->query($sql);
    ...
    public function get_city_name_by_code($code)
    {
    	...
    	$sql="SELECT city_name FROM t_insurance_picclife_citycode WHERE city_code='$code'";
		$query = $_SGLOBAL['db']->query($sql);
    ...
    public function get_county_name_by_code($code)
    {
    	...
    	$sql="SELECT county_name FROM t_insurance_picclife_countycode WHERE county_code='$code'";
		$query = $_SGLOBAL['db']->query($sql);
    ...
    public function get_career_name_by_code($code)
    {
    	...
    	$sql="SELECT name_subclassification FROM t_insurance_picclife_careercode WHERE code='$code'";
		$query = $_SGLOBAL['db']->query($sql);
    ...
    //通过职业代码获取寿险的职业类别
    public function get_career_type_by_code($code)
    {
    	global $_SGLOBAL;

    	$sql="SELECT type_life_insurance FROM t_insurance_picclife_careercode WHERE code='$code'";
		$query = $_SGLOBAL['db']->query($sql);
    ...
    public function get_industry_name_by_code($code)
    {
    	...
    	$sql="SELECT industry_name FROM t_insurance_picclife_industrycode WHERE industry_code='$code'";
		$query = $_SGLOBAL['db']->query($sql);
    ...
    public function get_bank_name_by_code($code)
    {
    	...
    	$sql="SELECT bank_name FROM t_insurance_picclife_bankcode WHERE bank_code='$code'";
		$query = $_SGLOBAL['db']->query($sql);
