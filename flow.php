...
//echo "sddddddddddddd: ---".$_REQUEST['step'];
//echo "sddddddddddddd:allow_direct_visit ---".$_REQUEST['allow_direct_visit'];



if(!isset($_REQUEST['allow_direct_visit']))
{	
...
//include_once('./baoxian/source/function_debug.php');
...
{//comment by wangcya , 20141225,选择支付方式的时候，要得到商品的总价格。
    /*------------------------------------------------------ */
    //-- 改变支付方式
    /*------------------------------------------------------ */
 ...
 /*   if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
    ...
    {*/
        ...
        $bx_order=$_SESSION['bx_order'];//comment by wangcya, 20141102,保存到session中也有问题吧？
        // 计算订单的费用 
        //start add by wangcya , 20141102,session丢失的处理
        if(empty($bx_order[0]['total_premium']))
        {
        	ss_log("session 丢失了总保费，退出!");
        	show_message("您的支付过程出现session异常");
        }
        //end add by wangcya , 20141102,session丢失的处理
        
        $total_premium = $bx_order[0]['total_premium'];
        */
        $total_premium = $_SESSION['total_premium'];//add by wangcya, for bug[193],能够支持多人批量投保,总保费
        
        ss_log("in flow select_payment, total_premium: ".$total_premium);
        
        $total = bx_order_fee($order, $total_premium, $consignee);
...
elseif ($_REQUEST['step'] == 'change_surplus')
{
    ...
    else
    {
        ...
        else
        {
            ...
            /*
            $bx_order=$_SESSION['bx_order'];
            $order['amount']=$bx_order[0]['totalModalPremium'];
            //start add by wangcya , 20141102,session丢失的处理
            if(empty($bx_order[0]['total_premium']))
            {
            	ss_log("session 丢失了总保费，退出!");
            ...
            */
            $total_premium = $_SESSION['total_premium'];//add by wangcya, for bug[193],能够支持多人批量投保,总保费
            ss_log("in flow change_surplus, total_premium: ".$total_premium);
            //comment by zhangxi, 20150127, 这里也得考虑啊，否则有问题
            $total = bx_order_fee($order, $total_premium, $consignee);
            $smarty->assign('total', $total);            
            $result['content'] = $smarty->fetch('library/order_total.lbi');
...
/*add yes123 2015-07-06 改变金币*/
elseif ($_REQUEST['step'] == 'change_service_money')
{
    /*------------------------------------------------------ */
    //-- 改变余额
    /*------------------------------------------------------ */
    include_once('includes/cls_json.php');
    $service_money   = floatval($_GET['service_money']);
    $user_info = user_info($_SESSION['user_id']);
    
    if ($user_info['service_money'] + $user_info['credit_line'] < $service_money)
    {
        $result['error'] = $_LANG['service_money_not_enough'];
    }
    else
    {
        /* 取得购物类型 */
...
/*     取得购物类型 
    ...
    $consignee = get_consignee($_SESSION['user_id']);
     ...
     对商品信息赋值 
    $cart_goods = cart_goods($flow_type); // 取得商品列表，计算合计
	$temp = json_encode($cart_goods);
	
	ss_log("cart_goods 数量:".$temp);
    if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
    {
        $result['error'] = $_LANG['no_goods_in_cart'];
    }
    ...
    /* 计算订单的费用 */
    $total_premium = $bx_order[0]['total_premium'];
    $total_premium = $_SESSION['total_premium'];
    ss_log("in flow change_surplus, total_premium: ".$total_premium);
    $total = bx_order_fee($order, $total_premium, $consignee);
    $smarty->assign('total', $total);            
    $result['content'] = $smarty->fetch('library/order_total.lbi');
...
elseif ($_REQUEST['step'] == 'check_surplus')
{
	...
	$user_info['user_money'] = $user_info['user_money']+$user_info['service_money']+$user_info['award_money'];
    if (($user_info['user_money'] + $user_info['credit_line'] < $surplus))
    {
...
//-- 完成所有订单操作，提交到数据库
/*------------------------------------------------------ */
elseif ($_REQUEST['step'] == 'done')//comment by wangcya , 20141011, 主要是余额支付
{
	...
	$ROOT_PATH__= str_replace ( 'flow.php', '', str_replace ( '\\', '/', __FILE__ ) ) ;
	...
	{	
		ss_log('session id :'.$_SESSION['user_id']);	
	...
	Eba_LogManager::log('data from flow.php for submit order .');
	//end
	...
	//add by yes123, 20150127
	$order_obj = new Order;
	$res = $order_obj->done();
	
	$insurer_code = isset($_REQUEST['insurer_code'])?trim($_REQUEST['insurer_code']):'';
...
//start add by wangcya,20141030
if(isset($_REQUEST['allow_direct_visit']) && $_REQUEST['allow_direct_visit'])
{
	if($_REQUEST['step']== "add_bx")
	{
		ss_log("检查余额");
		...
		if (($user_info['user_money'] + $user_info['credit_line'] < $surplus))
		{
			...
			$ret_attr['info']= "post policy fail! surplus not enough";
		...
		$_POST['step']= "done";
		$_POST['shipping'] = -1;
		$_POST['price_total'] = $_GET['price_total'];
		$_POST['policy_id'] = $_GET['policy_id'];
		$_POST['payment'] = 1;
		$_POST['goods_id'] = $_GET['gid'];
		$_REQUEST['goods_id'] = $_GET['gid'];
	...
	{
		ss_log("flow, return json!");
		
		/////////////////////////////////////////////////////////
		$my_order_id = $order['order_id'];
		$my_order_sn = $order['order_sn'];
		if($my_order_id)
		{
			$order_device_id = $_GET['order_device_id'];
						
			$sql="UPDATE t_order_device SET my_order_id='$my_order_id',my_order_sn='$my_order_sn' WHERE order_device_id='$order_device_id'";
			ss_log($sql);
			$GLOBALS['db']->query($sql);
		}
		else
		{
			ss_log("flow, my_order_id is null");
		}
		////////////////////////////////////////
		$ret_attr = array();
		...
		{
			$ret_attr['status']= 0;//"0"; modify by wangcya, 20141010
			$ret_attr['info']= "post policy success!";
		...
		{
			$ret_attr['status'] = $result_attr['retcode'];//"0"; modify by wangcya, 20141010
			$ret_attr['info']   = $result_attr['retmsg'];//"post policy fail!";
...
function getTiChengByOrder_id($new_order_id,$str){
	$sql="SELECT g.goods_id,o.goods_amount FROM bx_order_goods g,bx_order_info o WHERE o.order_id=".$new_order_id ." and o.order_id= g.order_id"; 
	$order_info = $GLOBALS['db']->getRow($sql);
	
	$sql = "SELECT '$str' FROM bx_goods  WHERE goods_id=".$order_info['goods_id'];
	$tcbl = $GLOBALS['db']->getOne($sql);
	return $tcbl*$order_info['goods_amount'];
}
