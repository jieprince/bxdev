...
$result['avatarUrls'] = str_replace('../',"",$result['avatarUrls']);
update_avatar($result['avatarUrls'][0]);
...
function update_avatar($avatar)
{
	...
	if($user_id)
	{
		...
		$sql = "UPDATE ".$GLOBALS['ecs']->table('users')." set avatar='$avatar' WHERE user_id = $user_id";
