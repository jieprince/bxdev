...
function get_cat_articles($cat_id, $page = 1, $size = 20 ,$requirement='')
{
    ...
    if ($requirement != '')
    {
        $sql = 'SELECT article_id, title, author, add_time, file_url, open_type' .
               ...
               ' ORDER BY article_type DESC, sort_order ASC';
    }
    else 
    {
 ...
 * 获得指定分类下的分类完整信息
 *
 * @param   String     $cat_name_arr  分类名称的数组
 *
 * @return  array
 */
function get_article_cat_list($cat_name_arr)
{
	$cat_name_str="";
	foreach ( $cat_name_arr as $key => $value ) {
       $cat_name_str.="'".$key. "'".",";
	...
	$sql="SELECT cat_id,cat_name FROM ". $GLOBALS['ecs']->table('article_cat') . " WHERE cat_name in(".$cat_name_str.") ORDER BY sort_order ASC,cat_id DESC ";
	

	$article_cat_list =$GLOBALS['db']->getAll($sql);
	
	foreach ($article_cat_list as $key1 => $article_cat ) 
	{
  		foreach ( $cat_name_arr as $key2 => $value )
  		{
       		if($article_cat['cat_name']==$key2)
       		{
       			$article_cat_list[$key1]['article_key'] = $value;
       		}
		}
 ...
 * 通过文章ID获取文章
 *
 * @param   integer     $article_id  分类名称的数组
 *
 * @return  array
 */
