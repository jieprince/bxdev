...
//add yes123 2015-05-29 提现规则时用到
$money_type_arr =array(
		'user_money'=>'充值',
		'service_money'=>'金币',
		'award_money'=>'奖励'
...
$income_type_list = array (
	ORDER_SERVICE_MONEY => '订单服务费',
	ORGAN_SERVICE_MONEY => '渠道管理费',
	RECOMMEND_SERVICE_MONEY=>'推荐服务费'
);

/*关于我们要查询的类型*/
$cat_names = "'平台资讯','常见问题','发展历程','关于我们','加入我们','联系我们'";
$cat_tag = array("平台资讯"=>"information",
				 "常见问题"=>"faq",
				 "发展历程"=>"progress",
				 "关于我们"=>"about",
				 "加入我们"=>"join_us",
...
$check_status_list = array(
				PENDING_CHECK_STATUS=>'待审核',
				SAVED_CHECK_STATUS=>'已保存',
				CHECKED_CHECK_STATUS=>'已审核',
				NO_PASS_CHECK_STATUS=>'不通过'
...
$bonus_type_list = array(
				...
				SEND_BY_PRINT=>'线下发放',
				SEND_BY_REG=>'注册新用户发放',
				SEND_BY_PARENT=>'新用户审核通过给推荐人发放',

);

$sms_template_list = array(
	REG_VERIFICATION_CODE_SMS=>49443,
	RESET_PASSWORD=>49445,
	WITHDRAWALS_VERIFICATION_CODE_SMS=>0,
	BINDING_PHONE_CODE_SMS=>64841,
);

$sms_str_list = array(
	REG_VERIFICATION_CODE_SMS=>'注册验证码',
	RESET_PASSWORD=>'重置密码',
	CHANGE_PHONE=>'更换手机号',
	WITHDRAWALS_VERIFICATION_CODE_SMS=>'提现验证码',
	TRANSFER_VERIFICATION_CODE_SMS=>'转账验证码',
	CHECK_SMS=>'审核短信',
	MANUALLY_SEND_SMS=>'手动发送',
	SERVICE_CHARGE_SEND_SMS=>'服务费发送',
	BONUS_SEND_SMS=>'代金券短信',
	VERIFICATION_CODE_SMS=>'其他'
);
 ...
 * @param   string  $cname   		购买用户
 * @param   string  $incoming_type  收入来源：订单服务费，渠道管理费，推荐奖励
 * @return  void
 */
 
function log_account_change2(
	$user_id, $user_money = 0, 
	$cname,$incoming_type,
	$frozen_money = 0, $rank_points = 0, 
	$pay_points = 0, $change_desc = '', 
	$change_type = ACT_OTHER,$order_sn,
	$policy_id=0)
{
	ss_log("into function: ".__FUNCTION__);
	ss_log(__FUNCTION__.",incoming_type:".$incoming_type);
	ss_log(__FUNCTION__.",change_desc:".$change_desc);
 ...
 * @param   string  $incoming_type  收入来源：订单服务费，渠道管理费，推荐奖励
 * @param   string  $order_sn       订单号
 * @param   int     $policy_id      保单号，注销保单时用到
 * @param   string  $amount_type    目前是提现时用到。用于注明，提现来自哪个账户 'user_money'=>'充值','service_money'=>'服务费','award_money'=>'奖励'
 * 								    1.提现账户（user_money）
									2.服务费账户（service_money）
									3.奖励账户（award_money）
									4.提现手续费（withdraw_poundage）
									5.提现个人所得税（income_tax）
									6.订单服务费（money_myself）
									7.推荐服务费（money_recommend）
									8.渠道服务费 (money_organization)
									9.第三方充值（money_recharge）
									10.订单支付扣除奖励（order_award_money）
									11.订单支付扣除服务费（order_service_money） 
									12.订单支付扣除充值（order_user_money）
 * 
 * 
 * 
 * @param   string  $account_name   要更新bx_users 表中的哪个账户，可选值有user_money，service_money，award_money
 * @return  void
 */
function log_account_change(
		...
		$pay_points = 0, $change_desc = '', 
		$change_type = ACT_OTHER,$order_sn='',
		$incoming_type='',$cname=0,
		$policy_id=0,$amount_type='',$account_name='user_money'
		)
{
	ss_log(__FUNCTION__.",user_id:".$user_id);
	ss_log(__FUNCTION__.",user_money:".$user_money);
	ss_log(__FUNCTION__.",change_desc:".$change_desc);
	ss_log(__FUNCTION__.",change_type:".$change_type);
	ss_log(__FUNCTION__.",incoming_type:".$incoming_type);
	ss_log(__FUNCTION__.",order_sn:".$order_sn);
	ss_log(__FUNCTION__.",amount_type:".$amount_type);
	ss_log(__FUNCTION__.",account_name:".$account_name);
	...
	ss_log(__FUNCTION__.",log_id:".$log_id);
    /* 更新用户信息 */
    $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
            " SET $account_name = $account_name + ('$user_money')," .
            " frozen_money = frozen_money + ('$frozen_money')," .
            " rank_points = rank_points + ('$rank_points')," .
            " pay_points = pay_points + ('$pay_points')" .
            " WHERE user_id = '$user_id' LIMIT 1";
 ...
 * @return  mix
 */
function article_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0,$position='shop')
{
    static $res = NULL;

    if ($res === NULL)
    {
        $data = read_static_cache('art_cat_pid_releate_'.$position);
        if ($data === false)
        {
        	$where= " WHERE c.position='$position' ";
            ...
            $res = $GLOBALS['db']->getAll($sql);
            write_static_cache('art_cat_pid_releate_'.$position, $res);
        }
        else
        {
            $res = $data;
    ...
    if (!isset($cat_options[0]))
    {
        ...
        while (!empty($arr))
        {
            ...
            if($i==1000)
            {
            	...
            	ss_log(__FUNCTION__.",循环分类出问题了。。".$i);
            	$arr=null;
            }
...
//add yes123 2015-01-13 处理最近时间
function sdtimeByZjTime() {
		global $_LANG;
		$data = array ();
		$recent = isset ($_REQUEST['recent']) ? trim($_REQUEST['recent']) : "";
		if ($recent != '') {
			require (ROOT_PATH . 'languages/zh_cn/user.php');
			$current_time = time();
			$old_time = "";
			//最近一周
			if ($recent == $_LANG['one_week']) {
				$old_time = $current_time - (86400 * 7);
			}
			elseif ($recent == $_LANG['one_month']) {
				//最近一个月
				$old_time = $current_time - (86400 * 30);
			}
			elseif ($recent == $_LANG['three_month']) {
				//最近三个月
				$old_time = $current_time - (86400 * 30 * 3);
			} else {
				//最近六个月
				$old_time = $current_time - (86400 * 30 * 6);
			}
			
			$data['s_add_time'] = date("Y-m-d H:i",$old_time);
			$data['e_add_time'] = date("Y-m-d H:i",$current_time);
		}
		return $data;
}
/**
...
function set_config()
{
	...
	if(!empty($distributor_cfg))
	{
		//自定义logo
		if($distributor_cfg['website_logo'])
		{
			$_CFG['shop_logo'] = $distributor_cfg['website_logo'];
		}
		//站点名称
		if($distributor_cfg['website_name'])
		{
			$_CFG['shop_name'] = $distributor_cfg['website_name'];
			$_CFG['shop_title'] = $distributor_cfg['website_name'];
		}
		//客服电话
		if($distributor_cfg['service_phone'])
		{
			$_CFG['service_phone'] = $distributor_cfg['service_phone'];
		}
		//客服邮箱
		if($distributor_cfg['service_phone'])
		{
			$_CFG['service_email'] = $distributor_cfg['service_email'];
		}
		
		//子站特殊标记
		$_SESSION['website_type'] = 'no_ebaoins';
		$_SESSION['is_iframe'] = $distributor_cfg['is_iframe'];
	...
	{
		$_SESSION['website_type'] = 'yes_ebaoins';
	}
	
	if(strstr($_SERVER["PHP_SELF"],'cp.php'))   //投保时
	{
		$_SESSION['shop_logo'] = $_CFG['shop_logo'];
	}
	else
	{
		$_SESSION['shop_logo'] = str_replace('../','', $_CFG['shop_logo']);
	}
		
		
	$_SESSION['organ_id'] = isset($distributor_cfg['d_uid'])?$distributor_cfg['d_uid']:0;	
	
	$_SESSION['shop_name'] = $_CFG['shop_name'];
	$_SESSION['service_phone'] = $_CFG['service_phone'];
	$_SESSION['service_email'] = $_CFG['service_email'];
	
	
	$_SESSION['shop_keywords'] = $_CFG['shop_keywords'];
	$_SESSION['shop_desc'] = $_CFG['shop_desc'];
	$_SESSION['shop_desc'] = $_CFG['shop_desc'];
...
//add yes123 2015-06-19 通过域名或者用户信息获取 子站配置
function get_organ_config()
{
	 ss_log(__FUNCTION__.",user_id:".$_SESSION['user_id']);
	 global $standard_addr;
	 
	 $where = " WHERE 1 ";
	 
	 $domain_name = $_SERVER['SERVER_NAME'];
	 if(!in_array($domain_name,$standard_addr)) //优先通过域名
	 {
	 	$www = "www.";
	 	$r = strpos($domain_name,$www);
	 	if($r===0)
	 	{
	 ...
	 else if(!empty($_SESSION['user_id'])) //通过用户信息
	 {
	 	$user = user_info_by_userid($_SESSION['user_id']);
	 	$id = $user['user_id'];
	 	$rank_code = $user['rank_code'];
	 	
	 
		if($rank_code!=ORGANIZATION_USER && $user['institution_id'])
	 	{
			$organ = user_info_by_userid($user['institution_id']);
	 		if($organ['rank_code']==ORGANIZATION_USER)
	 		{
	 			$rank_code = $organ['rank_code'];
	 	...
	 	if($rank_code==ORGANIZATION_USER)
		{
		 	$where.=" AND d_uid ='$id' ";
		}
		else
	 ...
	 //渠道子站配置表
 	 $sql = "SELECT *  FROM " .$GLOBALS['ecs']->table('distributor_cfg').$where;
	
 	 
 	 $distributor_cfg = $GLOBALS['db']->getRow($sql);
 	 return $distributor_cfg;
 ...
 * @return  array   用户信息
 */
function user_info_by_userid($id)
{
    if ($id == 0)
    {
        $id = $_SESSION['user_id'];
    ...
    //如果附件表没有，则查找不到
    $sql  = 'SELECT u.*,ui.*,d.* '.
    		' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u LEFT JOIN bx_user_info ui ON u.user_id=ui.uid ' .
    		" LEFT JOIN bx_distributor d ON u.user_id=d.d_uid WHERE u.user_id = '$id'";
    $user = $GLOBALS['db']->getRow($sql);
	
    $user['username']    = $user['user_name'];
    if(isset($GLOBALS['_CFG']['integral_name']))
    {
    ...
    if($user['user_money'])
    {
    	$user['format_user_money']  = price_format($user['user_money'], false);
    }
	
	if($user['user_rank'])
	{	
		$usre_rank = get_user_rank_info($user['user_rank']);
		$user['rank_code'] = $usre_rank['rank_code'];
	}
	else
	{
		$user['rank_code'] = COMMON_USER;
	}
...
//获取区域信息
function get_region_list($province_id=0,$city_id=0)
{
	$res = array();
	...
	if ($province_id) {
		...
		ss_log(__FUNCTION__.",通过省份ID获取城市列表：".$sql);
		$res['city_list'] = $city_list;
	}
	
	if($city_id)
	{
		$sql = "SELECT region_id, region_name FROM " . $GLOBALS['ecs']->table('region') . " WHERE parent_id = $city_id";
		$district_list = $GLOBALS['db']->getAll($sql);
		ss_log(__FUNCTION__.",通过城市ID获取区域列表：".$sql);
		$res['district_list'] = $district_list;
	}

	$res['province_list'] = $province_list;
...
function get_partner_code_bak($attribute_id=0,$user_id=0)
{
	...
	$partner_code = $GLOBALS['db']->getOne($sql);
	if($partner_code)
	{
		return $partner_code;
	}
	...
	ss_log(__FUNCTION__.",获取用户信息：".$sql);
	$user = $GLOBALS['db']->getRow($sql);
		
	$user_rank = get_user_rank_info($user['user_rank']);
	
	if($user_rank['rank_code']!=ORGANIZATION_USER && !$user['institution_id'])
	{
...
/*判断商品是否有权限购买*/
function is_buy_goods($attribute_id,$user_id)
{
	$flag=true;
	...
	{
		$organ_id = $user_info['user_id'];
	}
	else if($user_info['institution_id'])
	{
		$organ_id = $user_info['institution_id'];
	}
...
/*通过等级ID获取等级*/
function get_user_rank_info($rank_id=0,$rank_code='')
{
	$where_sql='';
	if($rank_id)
	{
		$where_sql = " WHERE rank_id = '$rank_id' ";
	}
	elseif($rank_code)
	{
		$where_sql = " WHERE rank_code='$rank_code'";
	}
	
	$sql = "SELECT * FROM bx_user_rank  $where_sql";
...
{
	$sql = "SELECT * FROM bx_user_rank ORDER BY min_points";
	$user_rank_list = $GLOBALS['db']->getAll($sql);
	return $user_rank_list;
	
}

//获取等级权限配置
function get_rank_cfg($rank_id,$rc_type)
{
	$sql = "SELECT * FROM bx_user_rank_config WHERE rank_id=$rank_id AND is_enabled=1 AND rc_type='$rc_type'";
...
function get_check_code()
{
	...
	$num = rand(1000,9999);
	$msg_type = isset($_REQUEST['msg_type'])?trim($_REQUEST['msg_type']):'common_msg';
	 
	$sql="SELECT user_id FROM ".$GLOBALS['ecs']->table('users'). " WHERE mobile_phone= '".$mobile."' OR user_name='$mobile'"; 
	$user_id = $GLOBALS['db']->getOne($sql);
	switch ( $type ) {
		case REG_VERIFICATION_CODE_SMS:
			if($user_id)
		 	{
		 	 	//响应json数据到客户端
		 	 	include_once (ROOT_PATH . 'api/EBaoApp/eba_adapter.php');
		 	 	EbaAdapter::responseData(RegisterResponseExist);
		 	 	//end add by dingchaoyang 2014-11-5
		 	 	//如果已经存在，就不发送短信，直接提示用户，此手机号码以被注册
		 	 	$datas = json_encode(array('code'=>GET_CHECK_CODE_FAIL,'msg'=>'此手机号码已被注册，请更换!')); //如果用户不存在,返回-2
		 		die($datas);
		 	 	
		 	}
		 	
		 	$send_type = REG_VERIFICATION_CODE_SMS;    
			$content = "您在本网站进行注册的验证码为：".$num;
		...
		case RESET_PASSWORD:
			if($user_id)
			{
				//add yes123 2015-03-23 判断此用户是否被禁用
			...
			{
				$datas = json_encode(array('code'=>GET_CHECK_CODE_FAIL,'msg'=>'用户不存在,请检查!')); //如果用户不存在,返回-2
		 		die($datas);
			}
		...
		case CHANGE_PHONE:
		 	$send_type = CHANGE_PHONE;
		 	$content = "您在本网站更换手机号的验证码为：".$num;
		 	break;
		case BINDING_PHONE_CODE_SMS:
			//校验是否手机号码已存在
			...
			if($res)
			{
		 	...
		 	$send_type = BINDING_PHONE_CODE_SMS;
		 	$content = "您在本网站绑定手机号的验证码为：".$num;
		 	break;
		case WITHDRAWALS_VERIFICATION_CODE_SMS:
		
			//检测是否有为处理的提现记录，如果有，不让用户重复发 ACT_SAVING
		    $sql = "SELECT id FROM " .$GLOBALS['ecs']->table('user_account')." WHERE user_id=".$_SESSION['user_id'] ." AND is_paid=0 AND process_type=".SURPLUS_RETURN." ";
	 ...
	 $send_data = array();
	 $send_data['mobile_phone'] = $mobile ; //目标手机号码
	 $send_data['msg'] = $content;//发送的内容，正对老短信接口的
	 $send_data['local_port'] = 1;//服务器端口
	 $send_data['send_type'] = $send_type;//发送验证码的类型，例如注册，还是找回密码等等
	 $send_data['verification_code'] = $num;//验证码
	 $send_data['sms_source'] = 2;//是老短信接口，还是新短信接口，2是新短信，1是老短信接口
	 $send_data['msg_type'] = $msg_type;//语音还是短信
	 $send_data['tempId'] = $sms_template_list[$send_type]; //模版ID
	 
	
 	 ss_log("start 用【新】短信接口发送。。");
 	 $result = send_message($send_data);
	 ...
	 //add yes123 2014-11-30  md5 加密后再返回给浏览器
	 $num = md5($num);
	 $result['num'] =$num;
 ...
 * @return  $combined
 */
function send_msg($mobile_phone,$msg,$local_port=0,$send_type='',$verification_code='',$sms_source=1)
{
	...
	{
		$useful_life = VCODE_USEFUL_LIFE/60;
		
		//云通讯发送短信
		include_once(ROOT_PATH.'sms/SendTemplateSMS.php');
		$send_res = sendTemplateSMS($mobile_phone,array($verification_code,$useful_life),$sms_template_list[$send_type]);
		
		$result = $send_res['code'];
	...
	$sms_data = array();
	$sms_data['session_id']  = SESS_ID;
	$sms_data['mobile_phone'] = $mobile_phone;
	$sms_data['send_content'] = $msg;//只能保存没有编码过的
	$sms_data['send_result']  = $result;
	$sms_data['send_type']  = $send_type;
	$sms_data['send_time']  = time();
	$sms_data['ip_address']  = getRealIP();
	$sms_data['verification_code']  = $verification_code;

	if(isset($_SESSION['admin_id']))
	{
		$sms_data['admin_id']  = $_SESSION['admin_id'];
	}
	
	$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('sms_send_log'), $sms_data, 'INSERT');
 ...
 * @return  $combined
 */
//function send_msg($mobile_phone,$msg,$local_port=0,$send_type='',$verification_code='',$sms_source=2)
function send_message($send_data)
{
	
	$mobile_phone = $send_data['mobile_phone']; //目标手机号码
	$msg = $send_data['msg'];//发送的内容，正对老短信接口的
	$local_port = $send_data['local_port'];//服务器端口
	$send_type = $send_data['send_type'];//发送验证码的类型，例如注册，还是找回密码等等
	$verification_code = $send_data['verification_code'];//验证码
	$sms_source = $send_data['sms_source'];//是老短信接口，还是新短信接口，2是新短信，1是老短信接口
	$msg_type = isset($send_data['msg_type'])?trim($send_data['msg_type']):'common_msg';//语音还是短信
	$tempId = $send_data['tempId'];
	

	
		 
	//发送验证码之前，判断是否操作频繁，每天，每一个手机号码，每个类型，每个IP，只能发送5次
	$t = time();
   	$start = mktime(0,0,0,date("m",$t),date("d",$t),date("Y",$t));
	$end = mktime(23,59,59,date("m",$t),date("d",$t),date("Y",$t));
   	
   	
   	//当天，同一个IP，同一个手机号码，同一个行为，不能超过10条
   	$ip = getRealIP();
	$where = "WHERE send_type='$send_type' AND send_time>'$start' AND send_time<'$end' AND (ip_address= '$ip' OR session_id='".SESS_ID."') ORDER BY id DESC LIMIT 50" ;
	$sql ="SELECT * FROM bx_sms_send_log  $where ";
	ss_log(__FUNCTION__.",获取之前发过的验证码：".$sql);
	$sms_send_log = $GLOBALS['db']->getAll($sql);
	if(count($sms_send_log)>ALLOW_GET_COUNT)
	{
		ss_log(__FUNCTION__.",获取验证码 次数超过：".ALLOW_GET_COUNT.",".$sql);
		return array('code'=>CODE_GET_OUT,'msg'=>'操作太频繁，请联系客服');;
	}
	...
	{
		//云通讯发送短信
		include_once('sms/SendTemplateSMS.php');
		ss_log(__FUNCTION__.",SendTemplateSMS  mobile_phone:$mobile_phone,verification_code：$verification_code,useful_life:$useful_life,temp_id:".$sms_template_list[$send_type]);
		$result = $send_res = sendTemplateSMS($mobile_phone,array($verification_code,$useful_life),$tempId);
		if($send_res['code']!=0)
		{
			//发送不成功，通知管理员
			$error_msg ="发送短信失败,error_code:".$send_res['code'].",error_msg:".$send_res['msg'];
			//send_msg(MANAGER_PHONE,'发送短信失败,error_code:'.$result['error_code'],1,$send_type,0000);
	...
	elseif($msg_type=='voice_msg')//发送语音信息
	{
		include_once('sms/VoiceVerify.php');
		voiceVerify($verification_code,3,$mobile_phone,null,null,'zh','第三方私有数据',null,null);
...
function check_code($mobile_phone,$checkCode,$send_type)
{
	
	ss_log(__FUNTION__.",mobile_phone:$mobile_phone,checkCode=$checkCode,send_type=$send_type");
	
    //查询保存的验证码
	$where = "WHERE  mobile_phone='$mobile_phone' AND send_type='".$send_type."' ORDER BY id DESC LIMIT 1" ;
	$sql ="SELECT * FROM bx_sms_send_log  $where ";
	ss_log(__FUNCTION__.",获取验证码对比：".$sql);
	$sms_send_log = $GLOBALS['db']->getRow($sql);
	
	$res = array();
	$res['code']=CHECK_CODE_OK;
	$res['msg']='';
	//add yes123 2014-11-28 验证用户注册信息 start
	if($sms_send_log)
	{
		
		ss_log("send_time:".$sms_send_log['send_time']);
		ss_log("verification_code:".$sms_send_log['verification_code']);
		
		$time = time();
		$time -= VCODE_USEFUL_LIFE;
		if($time>$sms_send_log['send_time'])
		{
			$res['msg'] = "短信动态验证码已过期，请重新获取！";
			$res['code']=CHECK_CODE_TIMEOUT;
		}
		
		//验证用户填写的验证码和服务端发送的是否一致
		if($sms_send_log['verification_code']!=$checkCode)
		{
			$res['msg'] = "短信动态验证码有误";
			$res['code']=CHECK_CODE_ERROR;
		}
		else
		{
			$res['msg'] = "短信动态验证码正确";
 ...
 * $arrays 要排序的数组
 * $sort_key 要排序的key
 * $sort_order      可选。规定排列顺序。可能的值是 SORT_ASC 和 SORT_DESC。
 * $sort_type    	可选。规定排序类型。可能的值是SORT_REGULAR、SORT_NUMERIC和SORT_STRING。
 * 
 * */
...
function get_payment_order($order)
{
	...
	if($user['renk_code']==ORGANIZATION_USER)	
	{
	...
	if($institution_id)
	{
		$sql = " SELECT * FROM ".$GLOBALS['ecs']->table('payment_organ')." WHERE user_id='$institution_id' AND pay_id='$pay_id' AND enabled=1 ";
...
function curl_post($url, $post_data) 
{
	...
	ss_log(__FUNCTION__.",post_data:".print_r($post_data,true));
