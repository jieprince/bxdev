...
class cls_session
{
    ...
    function cls_session(&$db, $session_table, $session_data_table, $session_name = 'ECS_ID', $session_id = '')
    {
        ...
        $this->session_data_table = $session_data_table;//comment by zhangxi, 20150210,这个在哪里呢?
        ...
        {
            $this->session_id = $_COOKIE[$this->session_name];//cookie中获取session id
        }
        else
        {
        ...
        if ($this->session_id)
        {
            ...
            //comment by zhangxi, 20150210, session值的校验，看是否是合法的，没有经过篡改过的session
            if ($this->gen_session_key($tmp_session_id) == substr($this->session_id, 32))
            {
                $this->session_id = $tmp_session_id;//获取真正的session id
            }
            else
            {
                $this->session_id = '';
    ...
    }//end session construct


    function gen_session_id()
    {
    	//comment by zhangxi, 20150210, 随机生成session id
    ...
    {
    	$session_count = $this->db->getOne('SELECT COUNT(ip) FROM ' . $this->session_table . " WHERE ip = '" . $this->_ip . "'");
    ...
    function load_session()
    {
        ...
        if (empty($session))//session没有则创建一条
        {
            $this->insert_session();
        ...
        {
        	//comment by zhangxi, 20150210, session还没有到过期时间
            ...
            else
            {
                ...
                else//comment by zhangxi, 20150210, session已经过期了
                {
                    $this->session_expiry = 0;
                    $this->session_md5    = '40cd750bba9870f18aada2478b24840a';
                    $GLOBALS['_SESSION']  = array();
    ...
    function update_session()
    {
       ...
       // unset($GLOBALS['_SESSION']['user_id']);//comment by zhangxi, 20150210, 为什么要注释掉呢 ?
