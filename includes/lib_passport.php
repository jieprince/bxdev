...
function register($username, $password, $email, $other = array())
{
    ...
    /* 检查email del yes123 2014-11-27  注册时,取消填写邮件地址
    if (empty($email))
    {
	...
	*/
	...
	//modify yes123 2015-01-06 添加注册时间
	  //if (!$GLOBALS['user']->add_user($username, $password, $email))
	if (!$GLOBALS['user']->add_user($username, $password, $email,'','',time()))
    {
    ...
    else
    {
        ...
        //定义other合法的变量数组
        $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone', 'is_institution', 'active_id','user_rank','platform_id');
...
function send_regiter_hash ($user_id)
{
    ...
    {
        $sql = "UPDATE " . $GLOBALS['ecs']->table('users')  . " SET is_validated = 2 WHERE user_id='$user_id'";
        $GLOBALS['db']->query($sql);
        return true;
    }
    else
