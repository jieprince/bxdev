...
function update_user_info()
{
    ...
    /* 查询会员信息 */
    $time = date('Y-m-d');
    $sql = 'SELECT u.user_id,u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points, u.is_institution, '.
            ' IFNULL(b.type_money, 0) AS user_bonus, u.last_login, u.last_ip,u.check_status,u.avatar'.
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        $_SESSION['formated_last_time'] = date("Y-m-d H:i:s",$row['last_login']);
        $_SESSION['last_ip']     = $row['last_ip'];
        $_SESSION['login_fail']  = 0;
        $_SESSION['email']       = $row['email'];
        $_SESSION['check_status'] = $row['check_status'];
        $_SESSION['avatar'] = $row['avatar'];
        /*判断是否是特殊等级，可能后台把特殊会员组更改普通会员组*/
        if($row['user_rank'] >0)
        {
            $sql="SELECT * from ".$GLOBALS['ecs']->table('user_rank')."where rank_id='$row[user_rank]'";
            $user_rank = $GLOBALS['db']->getRow($sql);
            if($user_rank['special_rank']==='0' || $user_rank['special_rank']===null)
            {   
            ...
            $_SESSION['rank_code']  = $user_rank['rank_code'];
            
        }
        else
        {
        	 $_SESSION['rank_code']  = COMMON_USER;
        }
        ...
        {
            // 特殊等级
            $sql = 'SELECT rank_id, discount,rank_code FROM ' . $GLOBALS['ecs']->table('user_rank') . " WHERE rank_id = '$row[user_rank]'";
            if ($row = $GLOBALS['db']->getRow($sql))
            {
                $_SESSION['user_rank'] = $row['rank_id'];
                $_SESSION['discount']  = $row['discount'] / 100.00;
        ...
        if(isset($_SESSION['rank_code']))
        {
        	if ($_SESSION['rank_code'] == ORGANIZATION_USER) 
        	{

				$sql = "SELECT d_name FROM " . $GLOBALS['ecs']->table('distributor') .
				" WHERE d_uid='$_SESSION[user_id]' LIMIT 1";
				ss_log("get d_name:" . $sql);
				$d_name = $GLOBALS['db']->getOne($sql);
				$_SESSION['real_name'] = $d_name;
			}
        }
...
function get_user_info($id=0)
{
    ...
    //modify yes123 2015-02-04
    if(isset($user['user_bonus']))
    {
...
function upload_file($upload, $type)
{
    if (!empty($upload['tmp_name']))
    {
        ...
        if (!empty($ftype))
        {
            ...
            ss_log("target:".$target);
            if (!move_upload_file($upload['tmp_name'], $target))
            {
                $GLOBALS['err']->add($GLOBALS['_LANG']['upload_file_error'], 1);
				
				ss_log($GLOBALS['_LANG']['upload_file_error']);
                return false;
            }
            else
            {
        ...
        {
        	ss_log($GLOBALS['_LANG']['upload_file_type']);
 ...
 * @return string               上传图片名
 */
function my_upload_file($upload, $target)
{
	$res = array();
    if (!empty($upload['tmp_name']))
    {
        ...
        if (!empty($ftype))
        {
            ...
            ss_log("target:".$target);
            if (!move_upload_file($upload['tmp_name'], ROOT_PATH.$target))
            {
		        $res['code']=1;
		        $res['msg']=$GLOBALS['_LANG']['upload_file_error'];
                $GLOBALS['err']->add($GLOBALS['_LANG']['upload_file_error'], 1);
				ss_log($GLOBALS['_LANG']['upload_file_error']);
	       		return $res;
            }
        ...
        {
        	$res['code']=1;
		    $res['msg']=$GLOBALS['_LANG']['upload_file_type'];
        	ss_log($GLOBALS['_LANG']['upload_file_type']);
            $GLOBALS['err']->add($GLOBALS['_LANG']['upload_file_type'], 1);
						...
						)
{
	...
	if (PlatformEnvironment::isMobilePlatform()){
		include_once (ROOT_PATH . 'api/EBaoApp/responseData/ebaOtherError/eba_other_error.class.php');
		$response = new Eba_OtherError($content,$_REQUEST['command']);
		$result = $response->responseResult();
		exit ( iconv ( 'utf-8', 'gb2312//ignore', json_encode ( $result ) ) );
	}
	
	//add yes123 2015-01-19 如果是微信用js提示用户
	$is_weixin = isset ($_REQUEST['is_weixin']) ?trim($_REQUEST['is_weixin']):0; 
	if($is_weixin){
		 echo '<script language=javascript>alert("'.$content.'");</script>';exit;
	}
	
	//end by dingchaoyang 2014-11-7
	assign_template();
...
function assign_template($ctype = '', $catlist = array(),$position='middle')
{
	
	$config_data = array();
	$config_data['shop_name'] = $GLOBALS['_CFG']['shop_name'];

	$config_data['service_email'] = $GLOBALS['_CFG']['service_email'];
	$config_data['service_phone'] = $GLOBALS['_CFG']['service_phone'];
...
/*    $smarty->assign('ww',            explode(',', $GLOBALS['_CFG']['ww']));
    ...
    $smarty->assign('shop_logo',     $shop_logo);
    $config_data['shop_logo'] = $shop_logo;
    ...
    $navigator_list = get_navigator($ctype, $catlist,$position);
    
    $smarty->assign('navigator_list',        $navigator_list);  //自定义导航栏
   
    $config_data['navigator_list'] = $navigator_list;
...
function get_affiliate()
{
    ...
    else
    {
	    ...
	    if($parent_name){
	    	$sql = " SELECT user_id FROM ".$GLOBALS['ecs']->table('users'). " WHERE user_name='$parent_name' OR mobile_phone='$parent_name'";
	    	ss_log(__FUNCTION__."获取推荐人ID:".$sql);
 ...
 * @return  array         列表
 */
function get_navigator($ctype = '', $catlist = array(),$position='middle')
{
    $sql = 'SELECT * FROM '. $GLOBALS['ecs']->table('nav') . '
            WHERE ifshow = \'1\' AND parent_id=0 ORDER BY type, vieworder';
    $res = $GLOBALS['db']->query($sql);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
    	...
    	$sql="SELECT * FROM ".$GLOBALS['ecs']->table('nav')." WHERE parent_id='$row[id]' AND ifshow=1 ";
        $child_nav_list = $GLOBALS['db']->getAll($sql);
	...
	//add yes123 2015-09-21  获取企业站的底部
/*	$en_bottom = array();
	
	if($position=='en_middle')
	{
		include_once (ROOT_PATH . 'includes/class/commonUtils.class.php');
		
		//1.获取底部资讯中心
		foreach ( $navlist['en_middle'] as $key => $value ) 
		{
			if($value['name']=='企业服务')
			{
				$sql = "SELECT cat_id FROM ".$GLOBALS['ecs']->table('article_cat')." WHERE cat_name='$value[name]'";
				$cat_id = $GLOBALS['db']->getOne($sql);
				
				$sql = " SELECT article_id,title FROM ".$GLOBALS['ecs']->table('article')." WHERE cat_id='$cat_id' ORDER BY sort_order ASC,article_id DESC LIMIT 6";
				
				$article_list = $GLOBALS['db']->getAll($sql);
				
				foreach ( $article_list as $key => $article ) 
				{
       				$article_list[$key]['name']=$article['title'];
       				$article_list[$key]['url']="article.php?act=services&id=".$article['article_id'];
       				
				}
				
				$value['child_nav_list'] = $article_list;
				$en_bottom[] = $value;
				
				
			}
			elseif($value['name']=='保险产品')
			{
				$sql = "SELECT cat_id FROM ".$GLOBALS['ecs']->table('article_cat')." WHERE cat_name='$value[name]'";
				$sql = "SELECT cat_id FROM ".$GLOBALS['ecs']->table('article_cat')." WHERE parent_id IN($sql) ";
				$cat_id_list = $GLOBALS['db']->getAll($sql);
				$cat_ids = CommonUtils :: arrToStr($cat_id_list, "cat_id");
				
				$sql = " SELECT article_id,title FROM ".$GLOBALS['ecs']->table('article')." WHERE cat_id IN($cat_ids) ORDER BY sort_order ASC,article_id DESC LIMIT 6";
				$article_list = $GLOBALS['db']->getAll($sql);
				...
				{
       				$article_list[$key]['name']=$article['title'];
       				$article_list[$key]['url']="article.php?act=product_article&id=".$article['article_id'];
			...
			elseif($value['name']=='资讯中心' || $value['name']=='关于我们')
			{
				$en_bottom[] = $value;
			}
...
{
	if(isset($GLOBALS['_CFG']['licensed']))
	{
