...
class chinabank
{
    ...
    function get_code($order, $payment)
    {
        ...
        $data_vmoneytype    = 'CNY';
        //目前我们实际没有进行设置，比较大的风险漏洞
        $data_vpaykey       = trim($payment['chinabank_key']);//comment by zhangxi,20150304, 秘钥，商户后台配置
		...
		ss_log(__FUNCTION__.",key=".$data_vpaykey);
    ...
    function respond()
    {
    	ss_log(__FUNCTION__.", get in---chinabank------");
    	$name = basename(__FILE__, '.php');
    	ss_log(__FUNCTION__.", name=".$name);
        $payment        = get_payment($name);
        ...
        //added by zhangxi, 20150304, 为了定位网银支付出现问题而加的日志
        ss_log(__FUNCTION__.", get in, v_pmode=".$v_pmode);

        /**
         * 重新计算md5的值
         */
        //mod by zhangxi, 20150304, 修改成去除前后空格
        $key            = trim($payment['chinabank_key']);
        //$key            = $payment['chinabank_key'];
		ss_log(__FUNCTION__.",key=".$key);
        ...
        if ($v_md5str==$md5string)
        {
            ...
            if ($remark1 == 'voucher')
            {
            	ss_log(__FUNCTION__.",remark1 == voucher");
            ...
            if ($v_pstatus == '20')
            {
            	ss_log(__FUNCTION__.":chinabank into order_paid ,order_sn: ".$v_oid);
                /* 改变订单状态 */
                order_paid($v_oid);

                return true;
            }
            //mod by zhangxi, 20150304, 增加异常处理信息
            else
            {
            	ss_log(__FUNCTION__.",error, v_pstatus =".$v_pstatus);
            	return false;
            }
        }
        else
        {
        	ss_log(__FUNCTION__.",error, md5 check failed v_md5str=".$v_md5str.",md5string=".$md5string);
            return false;
        }
    }
}
