...
class integrate {
	...
	function logout() {
		...
		$_SESSION['openid'] = ''; //add yes123 2014-12-26 清空openid
	}
	...
	function add_user($username, $password, $email, $gender = -1, $bday = 0, $reg_date = 0, $md5password = '') {
		...
		/* 检查email是否重复 del yes123 2014-11-27  注册时,取消填写邮件地址
		...
		//add yes123 2014-12-01  判断如果用户名是手机号码,就填写到手机字段
		include_once (ROOT_PATH . 'includes/class/commonUtils.class.php');
		if (CommonUtils :: isPhoneNumber($post_username)) {
			$fields[] = 'mobile_phone';
			$values[] = $post_username;
		}

		//add platformid by dingchaoyang 2014-12-19 注册时，如果来自app，则给platform_id赋值
	...
	function remove_user($id) {
		...
		if ($this->need_sync || (isset ($this->is_ecshop) && $this->is_ecshop)) {
			...
			if ($col) {
				...
				$GLOBALS['db']->query($sql);
				$sql = "DELETE FROM " . $GLOBALS['ecs']->table('user_bonus') . " WHERE " . db_create_in($col, 'user_id'); //删除用户代金券
				$GLOBALS['db']->query($sql);
				$sql = "DELETE FROM " . $GLOBALS['ecs']->table('user_account') . " WHERE " . db_create_in($col, 'user_id'); //删除用户账号金额
				$GLOBALS['db']->query($sql);
				...
				//add yes123 2015-03-25 删除bx_user_info 和bx_distributor里的信息
				$sql = "DELETE FROM " . $GLOBALS['ecs']->table('user_info') . " WHERE " . db_create_in($col, 'uid'); //删除user_info个人
				$GLOBALS['db']->query($sql);
				$sql = "DELETE FROM " . $GLOBALS['ecs']->table('distributor') . " WHERE " . db_create_in($col, 'd_uid'); //删除distributor渠道
				$GLOBALS['db']->query($sql);
				$sql = "DELETE FROM " . $GLOBALS['ecs']->table('distributor_cfg') . " WHERE " . db_create_in($col, 'd_uid'); //删除distributor渠道
				$GLOBALS['db']->query($sql);
			}
		}
	...
	function check_user($username, $password = null) {
		...
		//ss_log('ecshop check_user ecshop.usertable.'.$this->user_table.",field_name:".$this->field_name);
		$post_username = $username;
		...
		if ($password === null) {
			...
			ss_log("integrate check_user " . $sql);
			return $this->db->getOne($sql);
		}
	}
	 ...
	 *
	 * @access  public
	 * @param   string  $mobile_phone   用户手机号码
	 *
	 * @return  boolean
	 */
	function check_mobile_phone($mobile_phone) {
		if (!empty ($mobile_phone)) {
			...
			$sql = "SELECT " . $this->field_id .
			" FROM " . $this->table($this->user_table) .
			" WHERE " . $this->field_mobile_phone . " = '$mobile_phone' ";
			if ($this->db->getOne($sql, true) > 0) {
	...
	function set_cookie($username = '', $remember = null) {
		...
		elseif ($remember) {
			...
			$sql = "SELECT user_id, password FROM " . $GLOBALS['ecs']->table($this->user_table) . " WHERE user_name='$username' LIMIT 1";
	...
	function set_session($username = '') {
		if (empty ($username)) {
		...
		} else { //add yes123 2014-11-19 添加查询真名

			//modify yes123 2014-12-29 通过用户名，手机，邮箱都可以登录 $this->user_table
			//            $sql = "SELECT user_id, password, email,is_cheack,real_name FROM " . $GLOBALS['ecs']->table($this->user_table)  .
			//            		" WHERE user_name='$username' OR mobile_phone='$username' OR email='$username' LIMIT 1";
			//modify yes123 2015-02-09 查询所有，判断如果openid和c_openid 有值，就设置到session
			$sql = "SELECT * FROM " . $GLOBALS['ecs']->table($this->user_table) .
			" WHERE user_name='$username' OR mobile_phone='$username' OR email='$username' LIMIT 1";
			$row = $GLOBALS['db']->getRow($sql);
			...
			if ($row) {
				...
				$_SESSION['b_openid'] = isset ($row['openid']) ? $row['openid'] : "";
				//登录类型,如果是C端登陆的，做标记，区分用户信息是C还是B端
				if ($this->user_table == 'users_c') {
					ss_log("login set login_type=c");
					$_SESSION['c_openid'] = isset ($row['c_openid']) ? $row['c_openid'] : "";
					$_SESSION['login_type'] = "c";
				} else {
					ss_log("login set login_type=b");
