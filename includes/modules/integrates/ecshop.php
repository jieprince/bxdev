...
class ecshop extends integrate
{
    ...
    function check_user($username, $password = null)
    {
        ...
        if ($password === null)
        {
			...
			ss_log("password is null :".$sql);
            return $this->db->getOne($sql);
        }
        else
        {
        	//modify yes123 2014-12-29 通过用户名，手机，邮箱都可以登录
            $sql = "SELECT user_id, password,is_disable,salt,ec_salt " .
                   " FROM " . $this->table($this->user_table).
            ...
            if (empty($row['salt']))
            {
                ...
                else
                {
                   ...
                   " WHERE user_name='$post_username'";
					...
					if($row['is_disable'])
					{	
						$err_msg = "<script type=text/javascript>alert('您的账户异常，已禁用，如需恢复请联系管理员!');history.go(-1);</script>";
						ss_log($post_username.",您的账户异常，已禁用，如需恢复请联系管理员!");
						//show_message($err_msg,'', 'user.php?act=login', 'error','',false);	
						die($err_msg);
					}
