...
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, c.add_time, g.shop_price AS org_price, '.
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$goods_list[$row['goods_id']]['add_time']    = $row['add_time'];
...
function insert_user_account($surplus, $amount)
{
    ...
    $pay_document = @$surplus['pay_document'];
    
    //add platformid by dingchaoyang 2014-12-19 则给platform_id赋值
    $ROOT_PATH__= str_replace ( 'includes/lib_clips.php', '', str_replace ( '\\', '/', __FILE__ ) ) ;
    ...
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
           ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, pay_id,payment, is_paid, bid,pay_document,platform_id,operation_timestamp,amount_type)'.
            " VALUES ('$surplus[user_id]', '', '$amount', '".time()."', 0, '', '$surplus[user_note]', '$surplus[process_type]','$surplus[payment_id]', '$surplus[payment]', 0, '$surplus[bid]','$pay_document','$platform_id','$surplus[operation_timestamp]','$surplus[money_type]')";
...
function get_paylog_id($surplus_id, $pay_type = PAY_SURPLUS)
{
    $sql = 'SELECT log_id FROM' .$GLOBALS['ecs']->table('pay_log').
           " WHERE order_id = '$surplus_id' AND order_type = '$pay_type' AND is_paid = 0 ORDER BY log_id DESC LIMIT 1";
 ...
 */
function get_online_payment_list($include_balance = true,$is_mobile=0)
{
...
function get_mypayment($user_id=0)
{
	
	if($user_id)
	{
		$sql = " SELECT * FROM ".$GLOBALS['ecs']->table('payment_organ')." WHERE user_id='$user_id'  AND enabled=1 ";
 ...
 * @return  array
 */
function get_account_log($user_id, $num, $start,$where)
{
    $account_log = array();
/*    $sql = 'SELECT * FROM ' .$GLOBALS['ecs']->table('user_account').
    ...
    if ($res)
    {
    	//add yes123 2014-12-12 获取所有支付方式，为了拿支付code
    	$sql = 'SELECT * FROM ' .$GLOBALS['ecs']->table('payment');
    	$payment_list = $GLOBALS['db']->getAll($sql);
  
        while ($rows = $GLOBALS['db']->fetchRow($res))
        {
        	//add yes123 2014-12-12 通过支付id获取pay_code 
        	$rows['pay_code']        = getPayCodeById($payment_list,$rows['pay_id']);
        	$rows['pay_document']        = $rows['pay_document'];
			...
			//add by dingchaoyang 2014-12-3 添加状态id
			$rows['statusID']           = $rows['is_paid'];
            /* 会员的操作类型： 冲值，提现 */
            if ($rows['process_type'] == 0)
            {
                $rows['type'] = $GLOBALS['_LANG']['surplus_type_0'];//充值
            }
            else
            {
                $rows['type'] = $GLOBALS['_LANG']['surplus_type_1'];//提现
            }
...
function get_user_default($user_id)
{
	...
	$row = user_info($user_id);
	ss_log(__FUNCTION__.",check_status:".$row['check_status']);
    $row['user_money'] = $row['user_money'];
    
    //获取用户其他信息 2014-09-27 yes123
    $sql = "SELECT check_result FROM " .$GLOBALS['ecs']->table('user_info'). " WHERE uid = '$user_id'";
    $check_result = $GLOBALS['db']->getOne($sql);
    
    $info = array();

    $info['check_result'] = $check_result;
    $info['check_status'] = $row['check_status'];
...
function get_comment_list($user_id, $page_size, $start)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$row['formated_add_time'] = date($GLOBALS['_CFG']['time_format'], $row['add_time']);
        if ($row['reply_time'])
        {
            //$row['formated_reply_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['reply_time']);
			$row['formated_reply_time'] = date($GLOBALS['_CFG']['time_format'], $row['reply_time']);
        }
...
/**
 * add yes123 2014-12-12 获取pay_code
 */
function getPayCodeById($payment_list,$pay_id){
	foreach ( $payment_list as $key => $payment ) {
       if($pay_id==$payment['pay_id'])
       {
       		return $payment['pay_code'];
       }
	}
}
