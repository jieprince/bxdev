...
function get_categories_tree($cat_id = 0)
{
    ...
    else
    {
        $parent_id = 0;
         ...
         //add yes123 2015-06-19 通过配置里获取分类，
         $distributor_cfg =  get_organ_config();
         if($distributor_cfg)
         {
         	if($distributor_cfg['cat_ids'])
         	{
	         	$where.=" AND cat_id IN($distributor_cfg[cat_ids]) ";
         	}
         	else
         	{
         		$where.=" AND cat_id =0 ";
...
{ 
    ...
    $time = gmtime();
    $sql = 'SELECT 
                 ipa.*, ipa.description AS in_description , /*add by wangcya , 20140802*/  
                 g.*,c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
            ...
            " LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
             "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
             " INNER JOIN t_insurance_product_attribute AS ipa ON ipa.attribute_id = g.tid". /*add by wangcya , 20140802*/            
    ...
    if ($row !== false)
    {
    	...
    	$row['goods_name']       = @$row['in_attribute_name'];
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
               'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img,g.tid ' .
            "FROM " . $GLOBALS['ecs']->table('goods') . ' AS g '.
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['id']           = $row['goods_id'];
        $goods[$idx]['tid']           = $row['tid']; //add yes123 2015-01-10 险种ID
        $goods[$idx]['name']         = $row['goods_name'];
        $goods[$idx]['brief']        = $row['goods_brief'];
        $goods[$idx]['market_price'] = price_format($row['market_price']);
        $goods[$idx]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
...
/**
 * add by wangcya , 20140802 
 *
 */
//function get_ins_product_list_by_attribute($attribute_id, $start, $end)
function get_ins_product_list_by_attribute($attribute_id)
{
	//////////////////////////////////////////////////////
	
	$sql="SELECT * FROM t_insurance_product_base AS pb  
	      LEFT JOIN t_insurance_product_additional AS pa 
	...
	while ($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		$arr_ins_product_list[] = $row;
	}
	
	return $arr_ins_product_list;
}
...
function favourable_info($act_id)
{
    ...
    //add yes123 查询费率回填
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('activity_rate') .
    ...
    if($activity_rate_list)
    {
    	foreach ($activity_rate_list as $key => $value ) {
       		...
       		if($value['rate_type']=='rate_myself')
       		{
       			$row['rate_myself'] = $value['rate_value'];
       			$row['myself_is_enabled'] = $value['is_enabled'];
       		}
       		
       		if($value['rate_type']=='rate_recommend')
       		{
       			$row['rate_recommend'] = $value['rate_value'];
       			$row['recommend_is_enabled'] = $value['is_enabled'];
       		}
       		
       		if($value['rate_type']=='rate_organization')
       		{
       			$row['rate_organization'] = $value['rate_value'];
       			$row['organization_is_enabled'] = $value['is_enabled'];
       		}
		}
    	
    }
    
    $row['rate_myself'] = isset($row['rate_myself'])?$row['rate_myself']:0;
    $row['rate_recommend'] = isset($row['rate_recommend'])?$row['rate_recommend']:0;
    $row['rate_organization'] = isset($row['rate_organization'])?$row['rate_organization']:0;
...
function get_hot_goods_list(){
	$action = $_REQUEST['act'];
	//modify yes123 2014-12-11条件重构
	$condition['sort'] =  $_REQUEST['sort'] == '' ? 'og_total' : trim($_REQUEST['sort']); 
	$condition['order']  = trim($_REQUEST['order'])==''?'desc':trim($_REQUEST['order']); 
	$condition['brand_id'] = $_REQUEST['brand_id'];
	$condition['goods_name'] = trim($_REQUEST['goods_name']);
	$condition['act'] = $action;
	$children=" ipa.interface_flag=1 ";
	
	if($condition['goods_name'])
	{
		$children.=" AND ipa.attribute_name LIKE '%".$condition['goods_name']."%'" ;
	}
	...
	}else if($condition['sort']=='og_total'){
		$smarty->assign('og_total_order', $condition['order']);
	}
    ...
    $goodslist = category_get_goods($children, "", "", "", "", $size, $page, $condition['sort'], $condition['order']);
