...
class FileUpload {
	...
	private $allowtype = array (
		'jpg','gif','png','jpeg','gif','doc','xls','txt','zip','ppt','tif','pdf','rar','docx','xlsx','pptx'
		
		
	); //设置限制上传文件的类型
	private $maxsize = 15728640; //限制文件上传大小（字节）
	...
	function upload($fileField,$new_path='') {
		if($new_path)
		{
			$this->path = $new_path;
			...
			if ($this->setFiles($name, $tmp_name, $size, $error)) {
				...
				if ($this->checkFileSize() && $this->checkFileType()) {
					...
					if ($this->copyFile()) {
						...
						$res_error['code']= 0;
						$res_error['msg']= '上传成功';	
						$res_error['file']= $this->getFileName();	
						return $res_error;
					} else {
						$return = false;
					}
