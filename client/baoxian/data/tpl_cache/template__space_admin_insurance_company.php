<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template//space_admin_insurance_company|template//header|template//space_admin_sidebar|template//footer', '1417753650', 'template//space_admin_insurance_company');?><!DOCTYPE html>
...
<input name="checkAll" onclick="selectAllCheckBox()" type="checkbox"></label></td>
        <td width="15%"><a href="/productAction.do?method=search&amp;order=code">编码</a></td>
        <td width="30%"><a href="/productAction.do?method=search&amp;order=name">名称</a></td>
        <td width="15%"><a href="/productAction.do?method=search&amp;order=abbrName">简称</a></td>
        <td width="15%"><a href="/productAction.do?method=search&amp;order=parent">上级</a></td>
...
<td width="15%">修改</td>
      ...
      <?php if(is_array($list)) { foreach($list as $key => $value) { ?>   
            ...
            <tr class="tdContent">
                ...
                <td>
<input name="ids[]" value="<?=$value['insurer_id']?>" type="checkbox">
</td>
              
                <td><a href="<?=$theurl?>&insurer_id=<?=$value['insurer_id']?>" ><?=$value['insurer_code']?></a></td>
              
                <td>
                <a href="<?=$theurl?>&insurer_id=<?=$value['insurer_id']?>" ><?=$value['insurer_name']?></a>
                </td>
                
                <td><?=$value['insurer_abbrname']?></td>
                
                <td>
                <a href="<?=$theurl?>&id=<?=$value['parent_id']?>"><?=$value['parent_name']?></a>
                </td>

<td>
                <a href="<?=$theurl?>&id=<?=$value['area_id']?>"><?=$value['area_name']?></a>
                </td>


 <td>
             <a href="cp.php?ac=admin_insurance_company&insurer_id=<?=$value['insurer_id']?>" target="_self"><img src="template/images/updateButton.gif" alt="修改">
            修改</a>
             </td>
