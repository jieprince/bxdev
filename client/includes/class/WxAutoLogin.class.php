...
class WxAutoLogin {
    //公众号开发者id
    private $appid = "wxea0c670f3b2e1a22";
    //公众号开发者密钥
    private $appsecret = "2d240f57f3a28527929f6d0f4104db97";
    ...
    private function wx_login_start($appid, $appsecret) {
        ...
        if (!array_key_exists('code', $_GET)) {
            ...
            if (array_key_exists('openid', $respond) && array_key_exists('access_token', $respond)) {
				...
				ss_log('get openid client openid:'.$openid);
				...
				if (!empty($db_user)) {
					...
					$_SESSION['check_status'] =  $db_user['check_status']; 
					...
					//获取openid后跳转
					$url="http://www.ebaoins.cn/client/user.php";
			 		header("Location: $url");
                }
