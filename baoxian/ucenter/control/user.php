...
function echo_result($code,$message)
{
	echo '<result>';
...
	    //wangcya ,20121115,receive pic
	function onrectavatar() 	{		$mobile_type ='web';		//add by wangcya ,20140419		if(stristr($_SERVER['HTTP_USER_AGENT'],'kangyq'))//add by wangcya , 20130521,来自我们自己的手机客户端
		{
			$mobile_type = 'myself';
		}		elseif(stristr($_SERVER['HTTP_USER_AGENT'],'Android'))		{			$mobile_type = 'android';
		}		elseif(stristr($_SERVER['HTTP_USER_AGENT'],'iPhone'))//add by wangcya, 2130104
		{
			$mobile_type = 'iphone';
		}		else 		{			$mobile_type = 'web';		}		////////////////////////////////////////////////////////////////////////		if($mobile_type != 'myself')		{
			@header("Expires: 0");	
			@header("Cache-Control: private, post-check=0, pre-check=0, max-age=0", FALSE);	
			@header("Pragma: no-cache");	
			header("Content-type: application/xml; charset=utf-8");	
		...
		$smallavatarfile = UC_DATADIR.'./avatar/'.$this->get_avatar($uid, 'small', $avatartype);		if($mobile_type != 'myself')		{
			...
			$smallavatar = $this->flashdata_decode(getgpc('avatar3', 'P'));		}		else		{			$bigavatar = $_FILES['avatar1'];			$middleavatar = $_FILES['avatar1'];			$smallavatar = $_FILES['avatar1'];		}		
		if(!$bigavatar || !$middleavatar || !$smallavatar) 		{			if($mobile_type == 'myself')
			{
				echo '<?xml version="1.0" encoding="utf-8"?>';
			    echo_result('1', 'one of avatar is empty!' );
				return;
			}
			else
		...
		if($mobile_type != 'myself')
		{			if($bigavatar)			{
	...
				if($smallavatar)
			{
				...
				@fclose($fp);			}		}		else		{						if($bigavatar)			{				$tmp_name = $bigavatar['tmp_name'];				@copy($tmp_name, $bigavatarfile);			}					if($middleavatar)
			{				$tmp_name = $middleavatar['tmp_name'];
				@copy($tmp_name, $middleavatarfile);			}						if($smallavatar)
			{				$tmp_name = $smallavatar['tmp_name'];
				@copy($tmp_name, $smallavatarfile);			}					}
		...
		if($success) 		{			//start add by wangcya,for bug[198]//////////////////////////
			$setarr = array();
			...
			$setarr['updatetime'] = "updatetime=$timestamp";//$_SGLOBAL[timestamp]
			$this->db->query("UPDATE ".UC_DBTABLEPRE_KANGYQ."space"." SET ".implode(',', $setarr)." WHERE uid='$uid'");
			//end add by wangcya,for bug[198]////////////////////////////
								if($mobile_type == 'myself')
			{				
				echo '<?xml version="1.0" encoding="utf-8"?>';
				echo_result('0', 'success upload!' );
				return;
			}
			else
			{			
