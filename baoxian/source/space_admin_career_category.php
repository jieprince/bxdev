...
$theurl ="space.php?do=admin_career_category";
...
if($id)
{
	...
	$sql = "SELECT * FROM ".tname('career_category')." WHERE $wheresql";
	...
	include_once template("space_admin_career_category_view");
}
else
{
	$wheresql = "1";
	$countsql = "SELECT COUNT(*) FROM ".tname('career_category')." WHERE $wheresql";
	//echo $countsql;
	//ss_log($countsql);
	...
	if($count)
	{
		...
		$sql = "SELECT * FROM ".tname('career_category')." WHERE $wheresql LIMIT $start,$perpage";
	...
	include_once template("space_admin_career_category");
}
