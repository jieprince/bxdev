...
$theurl ="space.php?do=admin_insurance_additional";
...
if($id)
{
	...
	$sql = "SELECT * FROM ".tname('insurance_additional')." WHERE $wheresql";
	...
	include_once template("space_admin_insurance_additional_view");
}
else
{
	$wheresql = "1";
	$countsql = "SELECT COUNT(*) FROM ".tname('insurance_additional')." WHERE $wheresql";
	//echo $countsql;
	//ss_log($countsql);
	...
	if($count)
	{
		...
		$sql = "SELECT * FROM ".tname('insurance_additional')." WHERE $wheresql LIMIT $start,$perpage";
	...
	$_TPL['css'] = "admin_product";
	include_once template("space_admin_insurance_additional");
}
