...
$theurl ="space.php?do=admin_sales_channel";
...
if($id)
{
	...
	$sql = "SELECT * FROM ".tname('sales_channel')." WHERE $wheresql";
	...
	include_once template("space_admin_sales_channel_view");
}
else
{
	...
	$wheresql = "1";
	$countsql = "SELECT COUNT(*) FROM ".tname('sales_channel')." WHERE $wheresql";
	//echo $countsql;
	//ss_log($countsql);
	...
	if($count)
	{
		...
		$sql = "SELECT * FROM ".tname('sales_channel')." WHERE $wheresql LIMIT $start,$perpage";
	...
	include_once template("space_admin_sales_channel");
}
