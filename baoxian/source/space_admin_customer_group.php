...
$theurl ="space.php?do=admin_customer_group";
...
if($id)
{
	...
	$sql = "SELECT * FROM ".tname('customer_group')." WHERE $wheresql";
	...
	include_once template("space_admin_customer_group_view");
}
else
{
	$wheresql = "1";
	$countsql = "SELECT COUNT(*) FROM ".tname('customer_group')." WHERE $wheresql";
	//echo $countsql;
	//ss_log($countsql);
	...
	if($count)
	{
		...
		$sql = "SELECT * FROM ".tname('customer_group')." WHERE $wheresql LIMIT $start,$perpage";
	...
	include_once template("space_admin_customer_group");
}
