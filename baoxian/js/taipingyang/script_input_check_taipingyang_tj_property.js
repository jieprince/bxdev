...
$j(document).ready(function () {
	...
	*/
	$("input[name=applicant_type]").unbind("change");
	$("input[name=applicant_type]").change(function () {
             ...
             if ($selectedvalue == 1) //single
             {
                 ...
                 $('#more_upload_list').hide();
		 		 $("#as_temps").hide();
                 $j("select#relationshipWithInsured").val("0");
             ...
             else//group
             {
                 ...
                 $j("select#relationshipWithInsured").val("4");//其他
    ...
    $j("select#relationshipWithInsured").change(function () {
        ...
        if(val_this!="")
        {
            ...
            if(applicant_type_val==1){
                if(val_this==0){
                    $j("#as_temps").hide();
					$j("#div_assured_title").hide();
                }else{
					
					$j("#div_assured_title").show();
                    $j("#as_temps").show();
                }
    ...
    var pickerOpts = {
        ...
        onSelect: function (dateText, inst) {
			...
			if($('#p_attribute_type').val()=='tj_mazuiyiwai'){
	...
	var pickerOpts_2 = {
            ...
            dateFormat: 'yy-mm-dd',
            onSelect: function (dateText, inst){
            	$j("#endDate").val(dateText+" 00:00:00");
            }
        };

    $j("#startDate").datepicker(pickerOpts);
	if($('#p_attribute_type').val()=='tj_mazuiyiwai'){
		$j("#endDate").datepicker(pickerOpts_2);
	}
    ///////////////////////////////////////////////////
   ...
   // $j("#takeoffDate").datepicker(pickerOpts2);
    ...
    function check_buy_submit_taipingyang() {
        ...
        */
        ...
        //////////////////////////////////////////////////////////////////////
        var text_1 = $j("#applicant_province_code").find("option:selected").text();
        $j("#province_text").val(text_1);
		var text_2 = $j("#applicant_city_code").find("option:selected").text();
        $j("#city_text").val(text_2);
		var text_3 = $j("#applicant_county_code").find("option:selected").text();
        $j("#county_text").val(text_3);
...
function provice_city_Ajax(domid,val_type,type){
	...
	var datajson = {'action':'get_province_list'};
	...
	if(val_type=='city_by_province'){
		...
		datajson['action']='get_city_list';
		 
		tempname = 'city';
	}else if(val_type=='county_by_city'){
		tempname = 'county';
		datajson['city_code']=$('#'+sts_temp+'_city_code').val();
		datajson['action']='get_county_list';
	}
	$.ajax({
		 type: 'POST',
		 url: '../oop/business/get_cpic_tj_property.php' ,
		 async:false,
		 data: datajson ,
		 dataType: "JSON",
