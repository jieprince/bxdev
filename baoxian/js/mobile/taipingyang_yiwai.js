...
function submit_checked(){
	 ...
	 if(applicant_type_id==1 || applicant_type_id==2){
		 ...
		 if(applicant_type_id==1){
			 ...
			 if(!$('#text_select_province').data('province_id') || $('#text_select_province').data('province_id')==""){
				 showMessages('投保人省份没有选择!','text_select_province_P');
				 flag_submit = false;
				 return false; 
			 }
			 $('#applicant_province').val($('#text_select_province').data('province_name'));
			  
			 if(!$('#text_select_insureareaprov_code').data('insureareaprov_id') || $('#text_select_insureareaprov_code').data('insureareaprov_id')==""){
				 showMessages('投保人城市没有选择!','text_select_insureareaprov_code_P');
				 flag_submit = false;
				 return false; 
			 }
			 $('#applicant_city').val($('#text_select_insureareaprov_code').data('insureareaprov_name'));
		 ...
		 if(text_select_relationshipWithInsured!='1'){
				 ...
				 if(!$('#text_select_province_ass').data('province_id') || $('#text_select_province_ass').data('province_id')==""){
					 showMessages('被保险人省份没有选择!','text_select_province_ass_P');
					 flag_submit = false;
					 return false; 
				 }
				 $('#insured_province').val($('#text_select_province_ass').data('province_name'));
				 
				  
				 if(!$('#text_select_insureareaprov_ass_code').data('insureareaprov_id') || $('#text_select_insureareaprov_ass_code').data('insureareaprov_id')==""){
					 showMessages('被保险人城市没有选择!','text_select_insureareaprov_ass_code_P');
					 flag_submit = false;
					 return false; 
				 }
				 $('#insured_city').val($('#text_select_insureareaprov_ass_code').data('insureareaprov_name'));
				  
				 
				 if($.trim($('#insured_address').val())==""){
					 showMessages('被保险人详细地址不能为空!','insured_address');
					 flag_submit = false;
					 return false;
				 }
				 
				 if(!checkzipcode('insured_zipcode','app')){
					 showMessages('被保险人邮政编码有误!','insured_zipcode');
					 flag_submit = false;
					 return false;
				 }
