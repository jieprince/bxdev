...
function initSet(){
	...
	opt.default = {
		...
		onSelect: function(dateText, inst) { 
			$("#startDate").val(dateText.replace(/-/g, ""));
			var end_t = timeStamp2String(dateText,$('#period').val()); 
			$("#endDate").val((end_t.substring(0,10)).replace(/-/g, ""));
...
function showSelect_applicant_type_callback(){
	...
	if(applicant_type_str==1){
		...
		$('#text_select_relationshipWithInsured').data('relationshipWithInsured_name','被保人');
		$('#text_select_relationshipWithInsured').text('被保人');
...
function showSelect_relationshipWithInsured_callback(){
	...
	if(text_select_applicant_type_temp==1){
		if(relationshipWithInsured_id==5){
		...
		if(relationshipWithInsured_id==5){
			showMessages('"投保人类型"是机构的情况下"与被保险人关系"不可以选择"本人"','text_select_relationshipWithInsured_P'); 
			$('#text_select_relationshipWithInsured').data('relationshipWithInsured_id',"6");
			$('#text_select_relationshipWithInsured').data('relationshipWithInsured_name',"被保人");
			$('#text_select_relationshipWithInsured').text("被保人");
		}
...
//省份
function showSelect_province(domid,domPid,layertype){
	var str_data = $('#'+domPid).data('value');
	var initflag = true;
	if(layertype){ //判定 存在 不弹出选择框
		initflag = false;
...
function showSelect_province_callback(){
	...
	$.ajax({
		 ...
		 url: 'cp.php' ,
		 async:false,
		 data: {'ac':'product_buy_process_ajax_req','op':'huatai_city','provine_code':$('#text_select_province').data('province_id')} ,
		 dataType: "JSON",
		 success: function(data){
			...
			$(data).each(function(i, n){
				...
				html_pro_city.push({'insureareaprov_id':n.BD_Code,'insureareaprov_name':n.BD_Name});
			});
...
//出行目的地区域
function showSelect_destination_area(domid,domPid,layertype){
	var str_data = $('#'+domPid).data('value');
	var initflag = true;
	if(layertype){ //判定 存在 不弹出选择框
		initflag = false;
	}
	showSelect(initflag,domid,'destination_area_name',str_data,showSelect_destination_area_callback);
}
//出行目的地区域返回
function showSelect_destination_area_callback(){
	var destination_area_id = $('#text_select_destination_area').data('destination_area_id');
	$('#text_destination_country').data('destination_country_id','');
	$('#text_destination_country').data('destination_country_name','');
	$('#text_destination_country').text('');
	$.ajax({
		 type: 'GET',
		 url: 'cp.php' ,
		 async:false,
		 data: {'ac':'product_buy_process_ajax_req','op':'huatai_destination','area':destination_area_id} ,
		 dataType: "JSON",
		 success: function(data){
			var html_pro_city = [];
			...
			$(data).each(function(i, n){
				...
				html_pro_city.push({'destination_country_id':n.name,'destination_country_name':n.BD_Name});
			});
			$('#text_destination_country_P').data('value',JSON.stringify(html_pro_city));
		 }
	});	 
}
...
function submit_checked(){
	 ...
	 if(applicant_type_id==1 || applicant_type_id==2){
		 ...
		 if(applicant_type_id==1){
			  ...
			  if(!check_en_name('applicant_fullname_english','tbr','app')){
				  showMessages('投保人英文名不能为空或者格式有误!','applicant_fullname_english');
				  flag_submit = false;
				  return false;
			  }
			 ...
			 $('#insureareaprov_code').val($('#text_select_province').data('province_id'));
			 $('#insureareaprov').val($('#text_select_province').data('province_name'));
			 
			 if(!$('#text_select_insureareaprov_code').data('insureareaprov_id')){
				 showMessages('投保人城市没有选择!','text_select_insureareaprov_code_P');
				 flag_submit = false;
				 return false;
			 }
			 $('#insureareacity_code').val($('#text_select_insureareaprov_code').data('insureareaprov_id'));
			 $('#insureareacity').val($('#text_select_insureareaprov_code').data('insureareaprov_name'));
			 
			 if($.trim($('#applicant_address').val())==""){
				 showMessages('投保人详细地址不能为空!','applicant_address');
				 flag_submit = false;
				 return false;
			 }
			 
			 if(!checkzipcode('applicant_zipcode','app')){
				 showMessages('投保人邮政编码有误!','applicant_zipcode');
				 flag_submit = false;
				 return false;
			 }
		 ...
		 $('#relationshipWithInsured').val(text_select_relationshipWithInsured);
				 //被保险人验证
		 if(text_select_relationshipWithInsured!=5){
				 if(!check_applicant_name('assured_fullname','bbxr','app')){
					  showMessages('被保险人姓名3-24个字符!','assured_fullname');
					  flag_submit = false;
					  return false;
				 }
				 if(!check_en_name('assured_fullname_english','bbxr','app')){
					  showMessages('被保险人英文名不能为空或者格式有误!','assured_fullname_english');
					  flag_submit = false;
					  return false;
				  }
		  ...
		  if(!$('#text_select_purpose').data('purpose_id') || $('#text_select_purpose').data('purpose_id')=="" ){
			  showMessages('被保险人的出行目的没有选择!','text_select_purpose_P');
			  flag_submit = false;
			  return false;  
		  }
		  $('#Purpose').val($('#text_select_purpose').data('purpose_id')); 
		  
		  if(!$('#text_select_destination_area').data('destination_area_id') || $('#text_select_destination_area').data('destination_area_id')=="" ){
			  showMessages('被保险人的出行区域没有选择!','text_select_destination_area_P');
			  flag_submit = false;
			  return false;  
		  }
		  $('#destination_area').val($('#text_select_destination_area').data('destination_area_id')); 
		  
		  if(!$('#text_destination_country').data('destination_country_name') || $('#text_destination_country').data('destination_country_name')=="" ){
			  showMessages('被保险人的出行国家没有选择!','text_destination_country_P');
			  flag_submit = false;
			  return false; 
		  }
		  $('#destination_country').val($('#text_destination_country').data('destination_country_name')); 
		  
		  if($.trim($('#VisaCity').val())==""){
			  showMessages('被保险人签证办理城市没有填写!','VisaCity');
			  flag_submit = false;
			  return false;  
		  }
