...
function initSet(){
	...
	opt.default = {
		...
		onSelect: function(dateText, inst) { 
			...
			if($('#attribute_type').val()=='tj_mazuiyiwai'){
				$("#endDate").val('');
				var end_t = timeStamp2String(dateText,2,'day'); 
				var project_start_temp = end_t.substring(0,10).split('-'); 
				...
				opt4['default']['endYear'] = parseInt(project_start_temp[0])+20;
				opt4['default']['endMonth'] = (parseInt(project_start_temp[1]));
				opt4['default']['endDay'] = parseInt(project_start_temp[2]);
				$("#endDate").val('').scroller('destroy').scroller($.extend(opt4['date'], opt4['default']));  	
			}else{
				var end_t = timeStamp2String(dateText,$('#period').val()); 
				end_t = timeStamp2String(end_t,1,'day'); 
				$("#endDate").val(end_t.substring(0,10)+" 00:00:00");
			}
...
function showSelect_relationshipWithInsured_callback(){
	...
	if(text_select_applicant_type_temp==1){
		if(relationshipWithInsured_id==0){
		...
		if(relationshipWithInsured_id!=4){
...
function submit_checked(){
	 ...
	 if(applicant_type_id==1 || applicant_type_id==2){
		 ...
		 $('#relationshipWithInsured').val(text_select_relationshipWithInsured);
				 //被保险人验证
		 if(text_select_relationshipWithInsured!='0'){
