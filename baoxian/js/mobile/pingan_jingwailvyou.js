...
//出行目的地
function showSelect_jingwailvyou_country(domid,domPid,layertype){
	var str_data = $('#'+domPid).data('value');
	var initflag = true;
	if(layertype){ //判定 存在 不弹出选择框
		initflag = false;
	}
	showSelect(initflag,domid,'country_name',str_data,showSelect_jingwailvyou_country_callback);
}
//出行目的地返回
function showSelect_jingwailvyou_country_callback(){
	var destinationCountry_name = $('#text_select_jingwailvyou_country').data('country_id');
	
	 
	if(destinationCountry_name=='其他'){
		$('#destinationCountry_other').show();
		$('#destinationCountry').val('');
	}else{
		$('#destinationCountry').val(destinationCountry_name);
		$('#destinationCountry_other').hide();
	}
}
...
function submit_checked(){
	 ...
	 if(applicant_type_id==1 || applicant_type_id==2){
		 ...
		 if(applicant_type_id==1){
			 ...
			 if($('#text_applicant_group_certificates_type').data('applicant_certificates_type_id')!='01'){
		 ...
		 if(text_select_relationshipWithInsured!='1'){
		  ...
		  if(!$('#text_select_jingwailvyou_country').data('country_id') || $('#text_select_jingwailvyou_country').data('country_id')=="" ){
			  showMessages('被保险人的出行目的地没有选择!','text_select_jingwailvyou_country');
			  flag_submit = false;
			  return false;  
		  }else if($('#text_select_jingwailvyou_country').data('country_id')=='其他'){
			  if($.trim($('#destinationCountry').val())==""){
				  showMessages('被保险人的出行目的地其他说明没有填写!','destinationCountry');
				  flag_submit = false;
				  return false; 
			  }
