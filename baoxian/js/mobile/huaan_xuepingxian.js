...
function initSet(){
	...
	if(age_max_temp_date==18 || age_max_temp_date==6){
		age_max_temp_date = age_max_temp_date-1;
	}
	...
	opt3.default = {
		...
		mode: 'scroller', //日期选择模式
		lang:'zh',
		startYear:currdateNow.getFullYear()-50,//开始年份
		startMonth:(currdateNow.getMonth()),
...
function showSelect_certificates_type(domid,domPid,type,layertype){
	...
	if(type==1){
		...
		//showSelect(initflag,domid,'applicant_type_name',str_data,showSelect_certificates_type_callback);	
	}
}
...
function showSelect_relationshipWithInsured_callback(){
	...
	if(relationshipWithInsured_id=='601005'){
...
function submit_checked(){
	 ...
	 if($('#radio_zixun input:checked').val()!=2){
		  showMessages('您存在投保问询中任何一种情况都不可以投保!','radio_zixun');
		  flag_submit = false;
		  return false;
	 }
	 ...
	 if(applicant_type_id==1 || applicant_type_id==2){
		 ...
		 $('#relationshipWithInsured').val(text_select_relationshipWithInsured);
				 //被保险人验证
		 if(text_select_relationshipWithInsured!='601005'){
				 ...
				 var bbxr_bir = getAgeByBirthday($.trim($('#assured_birthday').val()),'assured_birthday');
			  ...
			  var bbxr_bir = getAgeByBirthday($.trim($('#applicant_birthday').val()),'assured_birthday');
		 ...
		 if(!$('#text_select_county').data('county_name')){
			 showMessages('被保险人地区没有选择!','text_select_county');
			 flag_submit = false;
			 return false;  
		 } 
		 if(!$('#school_name').val()){
			 showMessages('被保险人学校名称没有填写!','school_name');
			 flag_submit = false;
			 return false;   
		 }
		  
		 $('#assured_school').val('北京市'+$('#text_select_county').data('county_name')+$('#school_name').val()); 
		 if(!ydtychecked()){
