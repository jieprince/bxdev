...
$j(document).ready(function(){
	...
	$("select#applyNum").change(function(){
		 ...
		 var bz_01_Temp = accMul(1000,applyNum);
		 var bz_03_Temp = accMul(20000,applyNum);
		 var bz_05_Temp = accMul(6000,applyNum);
		 ...
		 $("span#showprice").html(totalpremium);
		  
		  $("input#totalModalPremium").val(totalpremium);
		  $('#baoer_temp').text(totalpremium);
		 $('#bz_001').text(bz_01_Temp);
		 $('#bz_002').text(bz_01_Temp);
		 $('#bz_003').text(bz_03_Temp);
		 $('#bz_004').text(bz_01_Temp);
		 $('#bz_005').text(bz_05_Temp);
		 //alert(totalpremium);
    ...
    date = parseInt(date);
	 ...
	 var bombay3 = date+(3600 * 24)*1;
      
	 var time3 = new Date(bombay3*1000);
     
	time3 = time3.getFullYear()+"-"+(time3.getMonth()+1)+"-"+time3.getDate();
	...
	//get_industry_info('assured_occupationClassCode','career',2);
	 ...
	 $j("a#btnNext").click(function(){
	   ...
	   $('#startDate').val(time3+' 00:00:00');
	   var age_ass_tempG = getAgeByBirthday($('#assured_birthday').val());
	    
	   $('#endDate').val(timeStamp2String(time3,parseInt(25-age_ass_tempG),'year'));
...
function check_buy_submit_xinhuashaoer(){
	...
	if(!check_cn_name('assured_fullname',2)){
		...
		return false;
	}else if(!check_All_card('assured_certificates_type','assured_certificates_code',2,true,'assured_birthday','assured[0][assured_sex]',false)){
		layer.alert("被保险人证件号码有误！",9,'温馨提示');
		window.setTimeout("document.getElementById('assured_certificates_code').focus();", 0)  ; 
		return false;
	}else if(!$('input[name="assured[0][assured_sex]"]:checked').val()){
	...
	var diff_Bday = DateDiff(dateBrthirdaytime,$('#assured_birthday').val());
	 
	if(diff_Bday<28){
		$('#assured_birthday_warn').attr("class","wrong_strong");
		$('#assured_birthday_warn').html("被保险人 28天--15岁之间");	
