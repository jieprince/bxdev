...
class usermodel {
	...
	function check_username($username) {
		...
		$len = strlen($username);		        //modify by wangcya, 20130110 for 15 to 25 ,for bug[313]
	...
	function add_user($username, $password, $email, $uid = 0, $questionid = '', $answer = '') {
		$salt = substr(uniqid(rand()), -6);		$passwordmd5 = md5($password);//add by wangcya , for bug[1076] 	����һ������IM��������̨������֤�Ĺ���
		$password = md5(md5($password).$salt);
		$sqladd = $uid ? "uid='".intval($uid)."'," : '';
		$sqladd .= $questionid > 0 ? " secques='".$this->quescrypt($questionid, $answer)."'," : " secques='',";
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."members SET $sqladd username='$username', password='$password', passwordmd5='$passwordmd5', email='$email', regip='".$this->base->onlineip."', regdate='".$this->base->time."', salt='$salt'");
	...
	function edit_user($username, $oldpw, $newpw, $email, $ignoreoldpw = 0, $questionid = '', $answer = '') {
		...
		$sqladd = $newpw ? "password='".md5(md5($newpw).$data['salt'])."'" : '';				$sqladd .= " ,passwordmd5='".md5($newpw)."'";//add by wangcya , for bug[1076] 	����һ������IM��������̨������֤�Ĺ���
