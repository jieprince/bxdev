...
class WxAutoLogin {
    // 公众号开发者id
    private $appid = "wxc3a5fd5be7cf1ad1"; //中天
    //公众号开发者密钥
    private $appsecret = "68293d2271ad5e8e7f982a112dbcbf6f";//中天
    ...
    private function wx_login_start($appid, $appsecret) {
        ...
        if (!array_key_exists('code', $_GET)) {
             ss_log("get openid not code  go get code..");
             
             $url = "http://" . $_SERVER['HTTP_HOST'] . "/mobile/getbopenid.php?act=getbopenid"; //微信服务器会执行这个url，就到init去了
	...
	private function autologin(){
            ...
            if (array_key_exists('openid', $respond) && array_key_exists('access_token', $respond)) {
                ...
                ss_log($this->appid.'-> get openid:'.$openid);
     ...
     * @param type $type
     */
    private function get_wx_code($appid, $type,$url) {
         
        //获取当前访问页面的url
        //$url = "http://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; //微信服务器会执行这个url，就到init去了
