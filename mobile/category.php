...
if ($c_id <= 0) {
	...
	if ($num > 0) {
		...
		foreach ($data as $key => $value) {
			$data[$key]['ins_product_list'] = get_ins_product_list_by_attribute($value['tid'], '', '');
			if($data[$key]['ins_product_list']){
			   $data[$key]['ins_product_list_select'] = $data[$key]['ins_product_list'][0];
			}
		}
