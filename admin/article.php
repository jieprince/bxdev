...
$position = isset($_REQUEST['position'])?trim($_REQUEST['position']):'shop';
...
$smarty->assign('act',     isset($_REQUEST['act'])?trim($_REQUEST['act']):'');
/*------------------------------------------------------ */
//-- 文章列表
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'list')
{
    /* 取得过滤条件 */
    $filter = array();
    $smarty->assign('cat_select',  article_cat_list(0,0,true,0,$position));
    $smarty->assign('ur_here',      $_LANG['03_article_list']);
    $smarty->assign('action_link',  array('text' => $_LANG['article_add'], 'href' => 'article.php?act=add&position='.$position));
...
if ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['03_article_list'], 'href' => 'article.php?act=list&position='.$position));
...
if ($_REQUEST['act'] == 'insert')
{
	...
	$article_img = basename($image->upload_image($_FILES['article_img'],'article_img'));
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, cat_id, article_type, is_open,is_popup, author, ".
                "author_email, keywords, content, add_time, file_url, open_type, link, description,article_img,position) ".
            "VALUES ('$_POST[title]', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]','$_POST[is_popup]', ".
                "'$_POST[author]', '$_POST[author_email]', '$_POST[keywords]', '$_POST[FCKeditor1]', ".
                "'$add_time', '$file_url', '$open_type', '$_POST[link_url]', '$_POST[description]','$article_img','$position')";
    $db->query($sql);
    ...
    $link[0]['href'] = 'article.php?act=add&position='.$position;

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'article.php?act=list&position='.$position;
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('article',     $article);
    $smarty->assign('cat_select',  article_cat_list(0, $article['cat_id'],true,0,$position));
    $smarty->assign('ur_here',     $_LANG['article_edit']);
    $smarty->assign('action_link', array('text' => $_LANG['03_article_list'], 'href' => 'article.php?act=list&position='.$position.'&' . list_link_postfix()));
...
if ($_REQUEST['act'] =='update')
{
	...
	if ((isset($_FILES['article_img']['error']) && $_FILES['article_img']['error'] == 0) || (!isset($_FILES['article_img']['error']) && isset($_FILES['article_img']['tmp_name']) && $_FILES['article_img']['tmp_name'] != 'none'))
	{
	 	$image->data_dir = 'images';
		$article_img = $image->upload_image($_FILES['article_img'],'article_img');
	}
	else
	{
		$article_img = $_REQUEST['old_article_img'];
		
	}
	
	
	
    if ($exc->edit("title='$_POST[title]', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]', is_popup='$_POST[is_popup]', is_open='$_POST[is_open]',author='$_POST[author]', author_email='$_POST[author_email]', keywords ='$_POST[keywords]', file_url ='$file_url', open_type='$open_type', content='$_POST[FCKeditor1]', link='$_POST[link_url]', description = '$_POST[description]',article_img='$article_img'", $_POST['id']))
    {
        $link[0]['text'] = $_LANG['back_list'];
        $link[0]['href'] = 'article.php?act=list&position='.$position.'&'  . list_link_postfix();
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'article.php?act=query&position='.$position.'&'  . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        ...
        if($_POST['type'] == 'merging_display')
        {
        	include_once (ROOT_PATH . 'includes/class/commonUtils.class.php');
        	
        	$merging_display_article['new_title']  = isset($_REQUEST['new_title'])?trim($_REQUEST['new_title']):'';
        	$merging_display_article['article_ids'] = implode(',',$_POST['checkboxes']);
        	$merging_display_article['add_time'] = time();
        	//获取这些文章的ID
        	$sql =" SELECT distinct cat_id  FROM ".$GLOBALS['ecs']->table('article')." WHERE article_id IN($merging_display_article[article_ids]) ";
        	$cat_id_list = $db->getAll($sql);
 			
        	$merging_display_article['cat_ids'] = CommonUtils::arrToStr($cat_id_list,'cat_id');
        	
        	$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merging_display_article'), $merging_display_article, "INSERT");  
    ...
    /* 清除缓存 */
    clear_cache_files();
    $lnk[] = array('text' => $_LANG['back_list'], 'href' => 'article.php?act=list&position='.$position);
...
/* 获得文章列表 */
function get_articleslist($position='shop')
{
    $result = get_filter();
    if ($result === false)
    {
        ...
        $filter['position'] = isset($_REQUEST['position'])?trim($_REQUEST['position']):'shop';
        ...
        if($filter['position'])
        {
        	$where.=" AND a.position='$filter[position]' ";
