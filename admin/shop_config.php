...
elseif ($_REQUEST['act'] == 'weixin')
{
    ...
    {        
        ...
        $db->autoExecute($ecs->table('weixinsetting'), $info, $_POST['handle'], empty($_POST['id'])?'':"id='$_POST[id]'");
        
        //写入缓存
        write_static_cache("shop_config-weixin",$info);
        
        sys_msg('操作成功');
    }  
    
    //添加UI,修改UI,
    $sql = "select * from " . $GLOBALS['ecs']->table('weixinsetting') . " order by add_time desc limit 1";
    $info = $db->getRow($sql);
    
    $smarty->assign('handle',    empty($info)?'INSERT':'UPDATE');    
    
    $info['add_time'] = $info['add_time']?local_date('Y-m-d H:i:s',$info['add_time']):'';
    $info['update_time'] = $info['update_time']?local_date('Y-m-d H:i:s',$info['update_time']):'';
    $smarty->assign('info',      $info);
        
    assign_query_info();

    $smarty->assign('ur_here',      '微信开发者配置');
    $smarty->assign('cfg', $arr[5]['vars']);
    $smarty->display('qrcode-weixin.htm');
}
/**
 * 设置系统设置
 *
