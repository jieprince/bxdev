...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    {
         /* 检查密码是否正确 */
         $sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt,mobile_phone,email".
    ...
    if ($row)
    {
        ...
        //缓存手机号码和email地址，开发票时用
        set_admin_session($row['user_id'], $row['user_name'], $row['action_list'], $row['last_login'],$row['mobile_phone'],$row['email']);
...
elseif ($_REQUEST['act'] == 'list')
{
	...
	$smarty->assign('role_array', get_role_list());
    /* 显示页面 */
    assign_query_info();
...
elseif ($_REQUEST['act'] == 'insert')
{
	admin_priv('admin_manage');
	//添加渠道
	if($_REQUEST['organization'] == 'insert'){
		...
		$isBad = 0;	
		    ...
		    if (!empty($_POST['user_name']))
		    {
		        ...
		        if (!$is_only)
		        {	
		        	$msg="该渠道用户名已存在,请更换其他用户名";
		            sys_msg("该渠道用户名已存在,请更换其他用户名<script> </script>", 0, $link);
		        }
		    }
		    ...
		    {	
		    	...
		    	$sql="select count(*) from bx_users where user_name='".$user_name."'";		    	
				$is_only = $db->getOne($sql);
		        if ($is_only!=0)
		        {	
		        	$msg="该账户已存在,请更换其他账户";
		            sys_msg("该账户已存在,请更换其他账户<script> </script>", 0, $link);
		        }
		    }
			...
			//管理员表
			$sql = "SELECT * FROM " . $ecs->table('role') . " WHERE role_name like '%渠道%'";
	        $organization_role= $db->getRow($sql);
	        $action_list = $organization_role['action_list'];
	        $role_id =  $organization_role['role_id'];
			$add_time = gmtime();
			$password  = md5($_POST['password']);
		    $sql = "INSERT INTO ".$ecs->table('admin_user')." (user_name, email, password, add_time, action_list,role_id,is_organization) ".
           "VALUES ('".trim($_POST['user_name'])."', '".trim($_POST['email'])."', '$password', '$add_time', '$action_list', '$role_id',1)";
			...
			$sql = "INSERT INTO ".$ecs->table('users')." (email, user_name, password, reg_time,is_institution) ".
	           "VALUES ('".trim($_POST['email'])."', '".trim($_POST['user_name'])."', '$password', '$add_time', '2')";
      		...
      		$arr=$db->getRow($sql);
      		$u_id= $arr['LAST_INSERT_ID()'];

      		$sql = "INSERT INTO ".$ecs->table('distributor')." (d_uid, d_name,d_email) ".
	           "VALUES ($u_id,'".trim($_POST['organization_name'])."','".trim($_POST['email'])."')";
      		...
      		if($isBad == 1){
			    ...
			    $sql = "DELETE FROM " . $ecs->table('admin_user') .
		        " WHERE user_id=".$u_id;
		   		$GLOBALS['db']->query($sql);
		   		sys_msg("$msg<script>alert('添加失败');window.location.href='privilege.php?act=organization_list'; </script>", 0, $link);
			}else{
				mysql_query('COMMIT');
				mysql_close($conn);
	      		sys_msg("添加成功<script>alert('添加成功');window.location.href='privilege.php?act=organization_list'; </script>", 0, $link);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT user_id, user_name, email, password, agency_id, role_id,mobile_phone FROM " .$ecs->table('admin_user').
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    //add yes123 2015-01-31 只有管理员才能看到管理员列表
...
/* 邮件设置密码 */
elseif($_REQUEST['act'] == 'email_affiliate'){
	 $msg="";
	 $successMsg="";	
	 $failMsg="";	
	 ...
	 /* 模板 */
	 $sql = 'SELECT template_subject, is_html, template_content FROM ' . $GLOBALS['ecs']->table('mail_templates') . " WHERE template_code = 'reset_password'";
     $tpl = $GLOBALS['db']->GetRow($sql); 
	 $template_subject = $tpl['template_subject'];
     $pass = 'baoxian'.rand(1000,9999);
	 $smarty->assign('password', $pass);
	 
	 $smarty->assign('user_name', $_REQUEST['user_name']);
     $content = $smarty->fetch('str:' . $tpl['template_content']);
	 	
	if (send_mail( "客户", $email_addrs, $template_subject, $content, $tpl['is_html']))
	{	
		$successMsg.=$email_addrs;
	}
	else
	{
		$failMsg.=$email_addrs;
	 ...
	 if($successMsg!=''){
	    $pass =  MD5($pass);
		$sql = "UPDATE " . $GLOBALS['ecs']->table('admin_user') . " SET  password = '$pass' WHERE user_id = '$_REQUEST[user_id]'";
		$GLOBALS['db']->query($sql);
...
{
    $list = array();
    $sql  = 'SELECT user_id, user_name, email, add_time, last_login, role_id '.
