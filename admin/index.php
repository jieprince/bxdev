...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    //add yes123 2014-12-15检查是否有error类型的日志，有的话，就提示管理员
    $sql = "SELECT * FROM " . $ecs->table('admin_log') . " WHERE log_level='error' AND is_delete=0 ";
    $row = $db->getAll($sql);    
    foreach ( $row as $value ) {
    	$del = "<a href='admin_logs.php?act=del_log&log_id=".$value['log_id']."']'>【处理】</a>";
    	// $warning[] = $value['log_info']." ".$del;  
	}
    
    
    //add yes123 2014-12-17 查看短信剩余条数
/*    $res = getSelSum();
    $re_req = "  <a href=index.php>【重新获取】</a>";
    $warning[] = $res.$re_req;  */
...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
    ...
    if(strpos(','.$_SESSION['action_list'].',', 'salesman_list') != 0){
    	header("Location: users.php?act=users_list_bydistributor_id&type=geren&distributor_id=".$_SESSION['admin_id']); 
    }
    else
    {
    	//start add by yes123 2014-11-13  添加判断，如果不是管理员，那么不显示起始页
    	if($_SESSION['action_list']=='all')
    	{ 
    		$smarty->display('start.htm');	
    	}
    	elseif($_SESSION['action_list']=='invoice_list')
    	{//发票功能，默认跳转到这里
    		//$smarty->display('start.htm');
      		header("Location: order.php?act=invoice_list");  		
    	}
    	//end by yes123 2014-11-13  添加判断，如果不是管理员，那么不显示起始页
...
/**
 * add yes123 2014-12-17 获取短信剩余数量
 * 
 */
function getSelSum()
{
