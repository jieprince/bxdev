...
//-- 批量发送邮件
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'mdrt_meeting_email'){
	 $msg="";
	 $successMsg="";	
	 $failMsg="";	
     ...
     send_mail( "客户","290605830@qq.com", "2015年首场MDRT名师公开课", $content, $tpl['is_html']);
     exit;
     $sql="SELECT user_id,email FROM " . $GLOBALS['ecs']->table('users');
     $user_list =  $GLOBALS['db']->getAll($sql);
     
	 foreach ($add_arr as $key => $value ) {
	 	
	    if(preg_match("/^(?:w+.?)*w+@(?:w+.?)*w+$/",$value['email']))
	    {
	       	if (send_mail( "客户", $value['email'], "2015年首场MDRT名师公开课", $content, $tpl['is_html']))
			{	
				$successMsg.=$value;
			}
			else
