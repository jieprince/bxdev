...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['add_new'], 'href' => 'navigator.php?act=add&position='.$position));
...
elseif ($_REQUEST['act'] == 'add')
{
    if (empty($_REQUEST['step']))
    {
		...
		if($position=='en')
		{
			$where.=" AND type IN('en_top','en_middle','en_bottom')";	
		}
		elseif($position=='shop')
		{
			$where.=" AND type IN('top','middle','bottom')";
		}
		
		elseif($position=='weixin')
		{
			$where.=" AND type IN('weixin')";
		}
		
        
        
        //获取所有导航
        $sql = "SELECT id,name,parent_id,type FROM ".$ecs->table('nav').$where;        
        $nav_list = $db->getAll($sql);
      	foreach ( $nav_list as $key => $value ) 
      	{
       		$nav_list[$key]['name'] = $value['name']."-".$_LANG[$value['type']];
		}
      	
      	
        $smarty->assign('nav_list',$nav_list);
        
        $smarty->assign('action_link', array('text' => $_LANG['go_list'], 'href' => 'navigator.php?act=list&position='.$position));
        $smarty->assign('ur_here', $_LANG['navigator']);
        assign_query_info();
    ...
    elseif ($_REQUEST['step'] == 2)
    {
		...
		$icon = $image->upload_image($_FILES['icon'],'icon_img');
        ...
        if($item_ifshow == 1 && $item_type == 'middle')
        {
            ...
            if($arr)
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type,parent_id,icon) VALUES('$item_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type','$parent_id','$icon')";
            }
        }

        if(empty($sql))
        {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,ifshow,vieworder,opennew,url,type,parent_id,icon) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type','$parent_id','$icon')";
        }
        $db->query($sql);
        clear_cache_files();
        $links[] = array('text' => $_LANG['navigator'], 'href' => 'navigator.php?act=list&position='.$position);
        $links[] = array('text' => $_LANG['add_new'], 'href' => 'navigator.php?act=add&position='.$position);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if (empty($_REQUEST['step']))
    {
        ...
        $rt['item_parent_id'] = $row['parent_id'];
        ...
        //获取所有导航
        $sql = "SELECT id,name,parent_id FROM ".$ecs->table('nav').$where;        
        $nav_list = $db->getAll($sql);
        
        foreach ( $nav_list as $key => $value ) {
       		if($value['parent_id']>0)
       		{
       			$nav_list[$key]['name']="&#12288;".$value['name'];
       		}
		}
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        if($arr)
        {
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('nav') .
                " SET name='$item_name',ctype='" . $arr['type'] . "',cid='" . $arr['id'] . "',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type',parent_id='$parent_id',icon='$icon' WHERE id='$id'";
        }
        else
        {
            //目标不是分类
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('nav') .
                " SET name='$item_name',ctype='',cid='',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type',parent_id='$parent_id', icon='$icon' WHERE id='$id'";
        }
...
elseif ($_REQUEST['act'] == 'del')
{
    ...
    $db->query($sql);
    clear_cache_files();
    ecs_header("Location: navigator.php?act=list&position=".$position);
    exit;
}
...
function get_nav()
{
    ...
    if($result === false)
    {
        ...
        $filter['position']   = empty($_REQUEST['position']) ? 'shop' : trim($_REQUEST['position']);
		
		
		$where = " WHERE parent_id=0 ";
		if($filter['position']=='en')
		{
			$where.=" AND type IN('en_top','en_middle','en_bottom')";	
		}
		elseif($filter['position']=='shop')
		{
			$where.=" AND type IN('top','middle','bottom')";
		}
		else if($filter['position']=='weixin')
		{
			$where.=" AND type IN('weixin')";
        ...
        /* 查询 */
        $sql = "SELECT id, name, ifshow, vieworder, opennew, url, type,parent_id".
    ...
    foreach($navdb as $k=>$v)
    {
        ...
        $sql="SELECT id, name, ifshow, vieworder, opennew, url, type,parent_id FROM ".$GLOBALS['ecs']->table('nav')." WHERE parent_id='$v[id]'";
        $child_nav_list = $GLOBALS['db']->getAll($sql);
        if(!empty($child_nav_list))
        {
        	foreach ( $child_nav_list as $key => $value ) {
	...
	//echo "<pre>";print_r($new_nav_list);
    $arr = array('navdb' => $new_nav_list, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
