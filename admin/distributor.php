...
if($action == 'organization_list')
{
    ...
    assign_query_info();
    $smarty->display('organization_list.htm');
}
...
elseif ($action == 'organization_edit')
{
	...
	$smarty->assign('zhengjian_type_list',      $zhengjian_type_list);
	//支付方式
	$sql = "SELECT po.*,p.pay_name,p.pay_desc FROM ".$GLOBALS['ecs']->table('payment_organ')."po INNER JOIN ".$GLOBALS['ecs']->table('payment')." p ON p.pay_id=po.pay_id WHERE user_id='$user_id'";
	
	$smarty->assign('payment_list', $GLOBALS['db']->getAll($sql));
    /* 显示页面 */
    assign_query_info();
    $smarty->display('organization_info.htm');
}
...
elseif ($action == 'organization_save')
{
	...
	$organ_cfg['is_iframe'] = isset($_REQUEST['is_iframe'])?intval($_REQUEST['is_iframe']):0;
...
elseif($action=='payment_add')
{
	$user_id = $_REQUEST['user_id'];

    $smarty->assign('ur_here',      "添加支付方式");
   
    $smarty->assign('action_link',      array('text' => '支付方式列表', 'href'=>'distributor.php?act=organization_edit&user_id='.$organ_id.'&click=payment'));
	...
	$smarty->assign('user_id',  $user_id);
	$smarty->assign('act_form',  'payment_insert');
	$smarty->display('organ_payment_info.htm');
}
...
elseif($action=='payment_edit')
{
	...
	$sql = "SELECT * FROM " . $ecs->table('payment_organ') . "WHERE id = '$id' ";
    $payment_organ = $db->getRow($sql);
    
    $sql = "SELECT * FROM " . $ecs->table('payment') . "WHERE pay_id = '$payment_organ[pay_id]' ";
    $payment = $db->getRow($sql);
	
	
    $smarty->assign('ur_here',      "编辑支付方式");
   
    $smarty->assign('action_link',      array('text' => '支付方式列表', 'href'=>'distributor.php?act=organization_edit&user_id='.$user_id.'&click=payment'));
	
	
	$payment_organ['pay_code'] = $payment['pay_code'];
	$pay_cfg = get_pay_cfg($payment_organ);
	
	$smarty->assign('user_id',  $user_id);
	$smarty->assign('pay_cfg',  $pay_cfg);
	$smarty->assign('act_form',  'payment_update');
...
elseif (isset($_POST['Submit']))
{
	...
	$act_form = isset($_REQUEST['act_form'])?trim($_REQUEST['act_form']):'payment_insert';
	$pay_id = isset($_REQUEST['pay_id'])?intval($_REQUEST['pay_id']):0;
	$user_id = isset($_REQUEST['user_id'])?intval($_REQUEST['user_id']):0;
	$pay_code = isset($_REQUEST['pay_code'])?intval($_REQUEST['pay_code']):'';
    ...
    $payment_organ_data = array(
    	'pay_config'=>$pay_config,
    	'enabled'=>1
    );
	
	
	if($act_form=='payment_insert')
	{
		
		$sql = "SELECT * FROM " . $ecs->table('payment') . "WHERE pay_id = '$pay_id'";
		$payment = $db->GetRow($sql);
  		
		$payment_organ_data['user_id'] = $user_id;
		$payment_organ_data['pay_id'] = $payment['pay_id'];
		$payment_organ_data['pay_code'] = $payment['pay_code'];
		$payment_organ_data['pay_name'] = $payment['pay_name'];
		
		
        $sql = "SELECT COUNT(*) FROM " . $ecs->table('payment_organ') . "WHERE pay_id = '$pay_id' AND user_id = '$user_id'";
        if ($db->GetOne($sql) > 0)
        {
        	sys_msg('支付方式已存在，不能重复添加！', 0, $link);
        	
        }

        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('payment_organ'), $payment_organ_data, "INSERT"); 
		
	}
	else if($act_form=='payment_update')
	{
		
		$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('payment_organ'), $payment_organ_data, "UPDATE", "user_id = $user_id AND pay_id='$pay_id' ");  
		
	}
    $link[] = array('text' => $_LANG['back_list'], 'href' => "distributor.php?act=organization_edit&user_id=$user_id&click=payment");
...
else if($action=='get_pay_config')
{
	...
	$act_form = isset($_REQUEST['act_form'])?trim($_REQUEST['act_form']):'';
	...
	{
		$payment_title = 'payment_organ';
		
	}
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table($payment_title) . " WHERE pay_code = '$_REQUEST[code]' AND enabled = '1'";
...
else if ($action == 'remove_payment')
{
	$id = $_REQUEST['id'];
	
	$sql = " DELETE FROM ".$GLOBALS['ecs']->table('payment_organ')." WHERE id=$id";
	$r = $GLOBALS['db']->query($sql);
...
/*渠道列表*/
function organization_list($organ_id=0)
{
    $result = get_filter();
    if ($result === false)
    {
...
function get_pay_cfg($pay)
{
	...
	$result['content'] = $smarty->fetch('organ_payment_cfg.htm');
	$result['error'] = '';
