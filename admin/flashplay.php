...
$flashImgPath =  'images/afficheimg/';

$flashplay_position= array('en_index'=>'企业站','shop'=>'商城','weixin'=>'微信端');
...
if ($_REQUEST['act']== 'list')
{
	...
	$smarty->assign('afficheimg_list',  $res['afficheimg_list']);
    ...
    $smarty->assign('action_link_special', array('text' => $_LANG['add_new'], 'href' => 'flashplay.php?act=add&position='.$position));
...
elseif($_REQUEST['act']== 'del')
{
    ...
    $id = (int)$_GET['id'];
	$sql =" DELETE FROM " . $GLOBALS['ecs']->table("afficheimg")." WHERE img_id='$id'";
	$db->query($sql);
    ecs_header("Location: flashplay.php?act=list&position=$position");
    exit;
}
elseif ($_REQUEST['act'] == 'add')
{
    ...
    if (empty($_POST['step']))
    {
        ...
        $smarty->assign('action_link', array('text' => $_LANG['go_url'], 'href' => 'flashplay.php?act=list&position='.$position));
        $smarty->assign('rt', $rt);
        $smarty->assign('ur_here', $_LANG['add_picad']);
        
        $smarty->assign('flashplay_position', $flashplay_position);
    ...
    elseif ($_POST['step'] == 2)
    {
        if (!empty($_FILES['img_file_src']['name']))
        {
            ...
            $target = ROOT_PATH.$flashImgPath . $name;
        ...
        {
        	$links[] = array('text' => '请上传图片', 'href' => 'flashplay.php?act=add&position='.$position);
        	sys_msg('请上传图片！', 0, $links);
        }
        
        
        $field_values = array();
        $field_values['img_url'] = isset($_POST['img_url'])?trim($_POST['img_url']):'';
        $field_values['img_text'] = isset($_POST['img_text'])?trim($_POST['img_text']):'';
        $field_values['img_sort'] = isset($_POST['img_sort'])?trim($_POST['img_sort']):0;
        $field_values['organ_id'] = isset($_POST['organ_id'])?trim($_POST['organ_id']):0;
        $field_values['position'] = isset($_POST['position'])?trim($_POST['position']):'';
        $field_values['img_file_src'] = $flashImgPath.$name;
        
  		$db->autoExecute($GLOBALS['ecs']->table('afficheimg'), $field_values, "INSERT");  
		
        $links[] = array('text' => $_LANG['go_url'], 'href' => 'flashplay.php?act=list&position='.$position);
...
elseif ($_REQUEST['act'] == 'edit')
{
	...
	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table("afficheimg")." WHERE img_id='$id'";
	$afficheimg = $GLOBALS['db']->getRow($sql);  
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        {
            //若链接地址为空
            $links[] = array('text' => $_LANG['return_edit'], 'href' => 'flashplay.php?act=edit&id=' . $id.'&position='.$position);
        ...
        {
			$field_values['img_file_src'] =$_POST['img_src'];;
        }
        else
        {
            $links[] = array('text' => $_LANG['return_edit'], 'href' => 'flashplay.php?act=edit&id=' . $id.'&position='.$position);
        ...
        $field_values['position'] = isset($_POST['position'])?trim($_POST['position']):'';
  		...
  		$db->autoExecute($GLOBALS['ecs']->table('afficheimg'), $field_values, "UPDATE","img_id='$id'"); 
...
function afficheimg_list()
{
    ...
    if ($result === false)
    {
		...
		$where = " WHERE 1 ";
		if($filter['position']=='en_index')
		{
			$where.=" AND position='en_index' ";	
		}
		elseif($filter['position']=='shop')
		{
			$where.=" AND position='shop' ";
		}
		elseif($filter['position']=='weixin')
		{
			$where.=" AND position='weixin'";
		}
		
		
		
		$record_count_sql = "SELECT count(img_id) FROM " . $GLOBALS['ecs']->table("afficheimg").$where;
		$filter['record_count'] = $GLOBALS['db']->getOne($record_count_sql);

        /* 分页大小 */
        $filter = page_and_size($filter);
		 
		$sql="SELECT * FROM  ".$GLOBALS['ecs']->table('afficheimg') .$where." LIMIT $filter[start] , $filter[page_size]";
    ...
    $arr = array('afficheimg_list' => $afficheimg_list, 'filter' => $filter,
