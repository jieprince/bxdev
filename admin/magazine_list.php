...
//comment by zhangxi, 20150317, 增加到杂志的发送队列中去
elseif ($_REQUEST['act'] == 'addtolist')
{
    $id         = intval($_REQUEST['id']);//杂志模板id
    $pri        = !empty($_REQUEST['pri']) ? 1 : 0;
    $start      = empty($_GET['start']) ? 0 : (int)$_GET['start'];
    $send_rank  = $_REQUEST['send_rank'];//发送邮件的范围
    ...
    {
        if (SEND_LIST == $rank_array['0'])//发送时普通优先级的
        {
        	//这个email列表可能需要修改了，我们可能没有使用bx_email_list
            ...
            if ($count > $start)
            {
                ...
                if ($add)//从满足条件的用户邮件列表中获取需要发送邮件的列表，放到需要发送邮件的列表中
                {
