...
if ($_REQUEST['act'] == 'list')
{
    ...
    $new_logdb=array();
    foreach ( $logdb['logdb'] as $value ) {
    	$sql="SELECT introducer_percent FROM" . $GLOBALS['ecs']->table('goods') . "WHERE goods_id=(
				SELECT goods_id FROM" . $GLOBALS['ecs']->table('order_goods') .  "WHERE order_id=".$value['order_id'].
			")";
		if($db->getOne($sql)>0){
			$new_logdb[]=$value;
		}	
            ...
            for ($i=0; $i < $num; $i++)
            {
               ...
               $$money=  getMoneyByOrder_id($oid);
                ...
                else
                {
                    ...
                    log_account_change($up_uid, $$money, 0, 0, 0, $info);
                    write_affiliate_log($oid, $up_uid, $row['user_name'], $$money, $setpoint, $separate_by);
                }
            }
        }
        else
...
function get_affiliate_ck()
{
    ...
    if(!empty($affiliate['on']))
    {
        ...
        {
            //推荐注册分成
            $sql = "SELECT o.*, a.log_id, a.user_id as suid,  a.user_name as auser, a.money, a.point, a.separate_type,u.parent_id as up FROM " . 
...
function getMoneyByOrder_id($order_id){
	
	$sql="SELECT goods_id,goods_amount FROM bx_order_goods WHERE order_id=".$order_id; 
	$order_info = $GLOBALS['db']->getRow($sql);
	
	$sql = "SELECT introducer_percent FROM bx_goods  WHERE goods_id=".$order_info['goods_id'];
	$introducer_percent = $GLOBALS['db']->getOne($sql);
	return $order_info['goods_amount']*$introducer_percent;
}
?>