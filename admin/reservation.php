...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('reservation_list.htm'), '', array('filter' => $msg_list['filter'], 'page_count' => $msg_list['page_count']));
}
/*------------------------------------------------------ */
//-- ajax 删除留言
/*------------------------------------------------------ */
...
function reservation_list()
{
    ...
    $where = ' AND f.msg_type='.M_RESERVATION;
    
    
    if ($filter['real_name'])
    {
        $where .= " AND f.real_name LIKE '%" . mysql_like_quote($filter['real_name']) . "%' ";
    }
    ...
    $sql = "SELECT f.msg_id, f.user_name, f.real_name,f.tel,f.msg_content,f.user_email,f.msg_title, f.msg_type, f.order_id, f.msg_status, f.msg_time, f.msg_area, COUNT(r.msg_id) AS reply " .
    ...
    {   if($value['order_id'] > 0)
        ...
        $msg_list[$key]['msg_time'] = date("Y-m-d H:i", $value['msg_time']);
