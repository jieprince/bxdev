...
if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
	...
	//获取专题列表，用于设置父专题 add by yes123 2014-11-15
	$list = get_topic_list();
	$smarty->assign('topic_list',  $list);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $parent_id= $_POST['parent_id']; //add by yes123 2014-11-13  父专题ID
	$add_time = gmtime();
    if ($is_insert)
    {
        $sql = "INSERT INTO " . $ecs->table('topic') . " (title,start_time,end_time,data,intro,template,css,topic_img,title_pic,base_style, htmls,keywords,description,parent_id,add_time)" .
                "VALUES ('$_POST[topic_name]','$start_time','$end_time','$data','$_POST[topic_intro]','$_POST[topic_template_file]','$_POST[topic_css]', '$topic_img', '$title_pic', '$base_style', '$htmls','$keywords','$description','$parent_id','$add_time')"; //add by yes123 2014-11-13  parent_id父专题ID
    }
    else
    {
        $sql = "UPDATE " . $ecs->table('topic') .
                "SET title='$_POST[topic_name]',start_time='$start_time',end_time='$end_time',data='$data',intro='$_POST[topic_intro]',template='$_POST[topic_template_file]',css='$_POST[topic_css]', topic_img='$topic_img', title_pic='$title_pic', base_style='$base_style', htmls='$htmls', keywords='$keywords',parent_id='$parent_id',description='$description'" .
...
function get_topic_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT t.*,p.title AS parent_title FROM " .$GLOBALS['ecs']->table('topic'). " AS t LEFT JOIN " .$GLOBALS['ecs']->table('topic'). " AS p  ON t.parent_id = p.topic_id ORDER BY $filter[sort_by] $filter[sort_order]";
