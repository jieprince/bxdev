...
elseif($_REQUEST['act'] == 'manage'){
   ...
   $sql = "SELECT * FROM " . $ecs->table('admin_user') . " WHERE role_id = '$role_id'";
   $manage_array = $db->getAll($sql);
   foreach($manage_array as $key=>$manage){
        $manage_array[$key]['add_time'] =  date('Y-m-d H:i:s', $manage['add_time']);
		if($manage['last_login'] != 0){
		  $manage_array[$key]['last_login'] = date('Y-m-d H:i:s', $manage['last_login']);
		}
		
   }
   $smarty->assign('admin_list', $manage_array);
   $smarty->display('role_manage.htm');
}
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('insurer_list',    $insurer_list); //add yes123 2014-11-27 获取保险公司
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('role')." (role_name, action_list, role_describe) ".
           "VALUES ('".$role_name."','$act_list','".trim($_POST['role_describe'])."')";
    ...
    {
    	//add yes123 2014-11-27 获取保险公司
    	insertOrUpdate_role_factor($insurer_code,$new_id,$role_name);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取角色信息 */
    $sql = "SELECT r.role_id, r.role_name, r.role_describe,rf.insurer_code FROM " .$ecs->table('role').
           " AS r LEFT JOIN " .$ecs->table('role_factor')." AS rf ON r.role_id = rf.role_id WHERE r.role_id = '".$_REQUEST['id']."'";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$ecs->table('role'). " SET action_list = '$act_list', role_name = '".$role_name."', role_describe = '".$_POST['role_describe']." ' ".
    ...
    {
    	//add yes123 2014-11-27 获取保险公司
    	insertOrUpdate_role_factor($insurer_code,$role_id,$role_name);
    	
    }
    else
    {
    	//add yes123 2014-11-27  如果$insurer_code为空,删除bx_role_factor表中对应数据
        $sql = "DELETE FROM " .$ecs->table('role_factor'). " WHERE role_id = '$role_id'";
       	$db->query($sql);
    }
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    else
    {
        ...
        //add by yes123 2014-11-27删除bx_role_factor表中对应的数据
        $sql = "DELETE FROM " .$ecs->table('role_factor'). " WHERE role_id = '$id'";
       	$db->query($sql);
...
{
	$insurer_sql = "SELECT insurer_id,insurer_name,insurer_code FROM t_insurance_company ";
    $insurer_list = $GLOBALS['db']->getAll($insurer_sql);
    return $insurer_list;
}	


function insertOrUpdate_role_factor($insurer_code,$role_id,$role_name)
{
	include_once (ROOT_PATH . 'baoxian/source/function_debug.php');
    //add yes123 2014-11-27 获取保险公司
    $role_factor_sql = "SELECT * FROM bx_role_factor WHERE role_id = ".$role_id;
	$role_factor = $GLOBALS['db']->getRow($role_factor_sql);
    
    $insurer_sql = "SELECT insurer_code,insurer_name FROM t_insurance_company WHERE insurer_code = '".$insurer_code."'";
	$insurer = $GLOBALS['db']->getRow($insurer_sql);
    
    if($role_factor)
    {
    	$sql = "UPDATE " .$GLOBALS['ecs']->table('role_factor'). " SET " .
     			" role_name = '" .$role_name."'," .
     			" insurer_code = '" .$insurer['insurer_code']."'," .
     			" insurer_name='".$insurer['insurer_name']."' WHERE role_id =".$role_id;
    }
    else
    {		
    	$sql = "INSERT INTO ".$GLOBALS['ecs']->table('role_factor')." (role_id, role_name, insurer_code,insurer_name) ".
	           "VALUES ('".$role_id."','".$role_name."','".$insurer['insurer_code']."','".$insurer['insurer_name']."')";
