 ...
 * EBAO 友情链接管理
 * ============================================================================
 * * 版权所有 2014-2015 北京保易通信息技术有限公司，并保留所有权利。
 * 网站地址: http://www.ebaoins.cn；
 * ----------------------------------------------------------------------------
 * 这不是一个自由软件！您只能在不用于商业目的的前提下对程序代码进行修改和
 * 使用；不允许对程序代码以任何形式任何目的的再发布。
 * ============================================================================
 * $Author: ebao $
 * $Id: partner_link.php 17217 2011-01-19 06:29:08Z ebao $
*/
...
$exc = new exchange($ecs->table('partner_link'), $db, 'link_id', 'link_name');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['add_link'], 'href' => 'partner_link.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('partner_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('partner_list.htm'), '',
...
{
    admin_priv('partner_list');

    $smarty->assign('ur_here',     $_LANG['add_link']);
    $smarty->assign('action_link', array('href'=>'partner_link.php?act=list', 'text' => $_LANG['list_link']));
    $smarty->assign('action',      'add');
    $smarty->assign('form_act',    'insert');

    assign_query_info();
    $smarty->display('partner_info.htm');
}
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($exc->num("link_name", $link_name) == 0)
    {
        ...
        {
        	$image->data_dir = IMAGE_DIR;
            $img_up_info = @basename($image->upload_image($_FILES['link_img'], 'afficheimg'));
            $link_logo   = IMAGE_DIR . '/afficheimg/' .$img_up_info;
        }
        ...
        /* 插入数据 */
        $sql    = "INSERT INTO ".$ecs->table('partner_link')." (link_name, link_url, link_logo, show_order) ".
        ...
        $link[0]['href'] = 'partner_link.php?act=add';

        $link[1]['text'] = $_LANG['back_list'];
        $link[1]['href'] = 'partner_link.php?act=list';
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT link_id, link_name, link_url, link_logo, show_order ".
           "FROM " .$ecs->table('partner_link'). " WHERE link_id = '".intval($_REQUEST['id'])."'";
    ...
    $smarty->assign('action_link', array('href'=>'partner_link.php?act=list&' . list_link_postfix(), 'text' => $_LANG['list_link']));
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ((isset($_FILES['link_img']['error']) && $_FILES['link_img']['error'] == 0) || (!isset($_FILES['link_img']['error']) && isset($_FILES['link_img']['tmp_name']) && $_FILES['link_img']['tmp_name'] != 'none'))
    {
    	$image->data_dir=IMAGE_DIR;
        $img_up_info = @basename($image->upload_image($_FILES['link_img'], 'afficheimg'));
        $link_logo   = ", link_logo = ".'\''. IMAGE_DIR . '/afficheimg/'.$img_up_info.'\'';
    }
    ...
    {
        //获取链子LOGO,并删除
        $old_logo = $db->getOne("SELECT link_logo FROM " .$ecs->table('partner_link'). " WHERE link_id = '$id'");
        if ((strpos($old_logo, 'http://') === false) && (strpos($old_logo, 'https://') === false))
        {
            $img_name = basename($old_logo);
            @unlink(ROOT_PATH . IMAGE_DIR . '/afficheimg/' . $img_name);
        }
    }
    ...
    /* 更新信息 */
    $sql = "UPDATE " .$ecs->table('partner_link'). " SET ".
            "link_name = '$link_name', ".
            "link_url = '$link_url' ".
            $link_logo.','.
    ...
    $link[0]['href'] = 'partner_link.php?act=list&' . list_link_postfix();
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'partner_link.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
function get_links_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 获得总记录数据 */
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('partner_link');
        ...
        /* 获取数据 */
        $sql  = 'SELECT link_id, link_name, link_url, link_logo, show_order'.
               ' FROM ' .$GLOBALS['ecs']->table('partner_link').
