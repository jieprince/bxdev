...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        if ($real_goods > -1)
        {
            //$where .= " AND is_real='$real_goods'"; //虚拟和真实商品一起取出来
        }
		 ...
		 *添加 tid 
		 */
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, tid, " .
