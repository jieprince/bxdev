...
//modify yes123 2014-12-09 添加日志类型 
//function admin_log($sn = '', $action, $content)
function admin_log($sn = '', $action, $content,$level='info')
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('admin_log') . ' (log_time, user_id, log_info, ip_address,log_level) ' .
            " VALUES ('" . time() . "', $_SESSION[admin_id], '" . stripslashes($log_info) . "', '" . real_ip() . "', '" . $level . "')";
 ...
 * 
 */
function admin_log2($log_info,$log_level='info')
{
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('admin_log') . ' (log_time, user_id, log_info, ip_address,log_level) ' .
            " VALUES ('" . time() . "', $_SESSION[admin_id], '" . $log_info . "', '" . real_ip() . "','".$log_level."')";
    $GLOBALS['db']->query($sql);
}
/**
 ...
 * @return  void
 */
function set_admin_session($user_id, $username, $action_list, $last_time,$mobile_phone="",$email="")
{
	
	//ss_log("admin_id: ".$user_id );
	
    $_SESSION['admin_id']    = $user_id;
    $_SESSION['admin_name']  = $username;
    $_SESSION['action_list'] = $action_list;
    $_SESSION['last_check']  = $last_time; // 用于保存最后一次检查订单的时间
    //add yes123 2015-01-29 缓存，开发票时用
    $_SESSION['mobile_phone']  = $mobile_phone; 
    $_SESSION['email']  = $email; 
    //ss_log("after admin_id: ". $_SESSION['admin_id'] );
}
