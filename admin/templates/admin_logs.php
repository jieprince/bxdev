...
elseif ($_REQUEST['act'] == 'query') {
	...
	make_json_result($smarty->fetch('admin_logs.htm'), '', array (
		'filter' => $log_list['filter'],
		'page_count' => $log_list['page_count']
	));
}

/*------------------------------------------------------ */
//add yes123 2014-12-15 删除日志
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'del_log') {
	$id = $_GET['log_id'];
	$sql = "UPDATE  " . $ecs->table('admin_log') . " SET is_delete=1 WHERE log_id = '$id'";
	$result = $db->query($sql);
...
function get_admin_logs() {
	...
	$filter['log_info'] = empty ($_REQUEST['log_info']) ? '' : trim($_REQUEST['log_info']);
	
	//add yes123 2014-12-09 按照日志类型查询
	$filter['level'] = empty ($_REQUEST['level']) ? '' : trim($_REQUEST['level']);
	...
	if ($filter['log_info'] != '') {
		$where .= " AND al.log_info LIKE '%" . $filter['log_info'] . "%'";
	}
	//add yes123 2014-12-09 按照日志类型查询
	if ($filter['level'] != '') {
		$where .= " AND al.log_level ='" . $filter['level'] . "'";
	}
