...
{
    $articlecat = article_cat_list(0, 0, false,0,$position);
    ...
    $smarty->assign('action_link', array('text' => $_LANG['articlecat_add'], 'href' => 'articlecat.php?act=add&position='.$position));
...
elseif ($_REQUEST['act'] == 'add')
{
	...
	$cat_select = article_cat_list(0, 0, true,0,$position);
	
    $smarty->assign('cat_select',  $cat_select);
	
    $smarty->assign('ur_here',     $_LANG['articlecat_add']);
    $smarty->assign('action_link', array('text' => $_LANG['02_articlecat_list'], 'href' => 'articlecat.php?act=list&position='.$position));
...
elseif ($_REQUEST['act'] == 'insert')
{
	...
	$articlecat_img = basename($image->upload_image($_FILES['articlecat_img'],'articlecat_img'));
    ...
    $sql = "INSERT INTO ".$ecs->table('article_cat')."(cat_name, cat_type, cat_desc,keywords, parent_id, sort_order, show_in_nav,position,articlecat_img)
           VALUES ('$_POST[cat_name]', '$cat_type',  '$_POST[cat_desc]','$_POST[keywords]', '$_POST[parent_id]', '$_POST[sort_order]', '$_POST[show_in_nav]','$position','$articlecat_img')";
    $db->query($sql);
    ...
    $link[0]['href'] = 'articlecat.php?act=add&position='.$position;

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'articlecat.php?act=list&position='.$position;
    clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT cat_id, cat_name, cat_type, cat_desc, show_in_nav, keywords, parent_id,sort_order,articlecat_img FROM ".
    ...
    $options    =   article_cat_list(0, $cat['parent_id'], false,0,$position);
    $select     =   '';
...
elseif ($_REQUEST['act'] == 'update')
{
	...
	if ((isset($_FILES['articlecat_img']['error']) && $_FILES['articlecat_img']['error'] == 0) || (!isset($_FILES['articlecat_img']['error']) && isset($_FILES['articlecat_img']['tmp_name']) && $_FILES['articlecat_img']['tmp_name'] != 'none'))
	{
	 	$image->data_dir = 'images';
		$articlecat_img = $image->upload_image($_FILES['articlecat_img'],'articlecat_img');
	}
	else
	{
		$articlecat_img = $_REQUEST['old_articlecat_img'];
    ...
    if ($exc->edit("cat_name = '$_POST[cat_name]', cat_desc ='$_POST[cat_desc]', keywords='$_POST[keywords]',parent_id = '$_POST[parent_id]', cat_type='$cat_type', sort_order='$_POST[sort_order]', show_in_nav = '$_POST[show_in_nav]', articlecat_img = '$articlecat_img'",  $_POST['id']))
    //if ($exc->edit("cat_name = '$_POST[cat_name]', cat_desc ='$_POST[cat_desc]', keywords='$_POST[keywords]',parent_id = '$_POST[parent_id]', cat_type='$cat_type', sort_order='$_POST[sort_order]', show_in_nav = '$_POST[show_in_nav]'",  $_POST['id']))
    {
        ...
        $link[0]['href'] = 'articlecat.php?act=list&position='.$position;
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'articlecat.php?act=query&position='.$position.'&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
