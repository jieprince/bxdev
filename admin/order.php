...
//ss_log("in admin order.php, admin_id: ".$_SESSION['admin_id'] );
//ss_log("in admin order.php, admin_name: ".$_SESSION['admin_name'] );
/*------------------------------------------------------ */
//-- 订单查询
/*------------------------------------------------------ */
...
if ($_REQUEST['act'] == 'updateReceiptInsurerCode')
{
	include_once (ROOT_PATH . 'includes/class/invoice.class.php');
	$invoice = new Invoice;
	$invoice->updateReceiptInsurerCode();
}

/*------------------------------------------------------ */
//-- 订单列表
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'bat_post_policy')
{//点击批量投保的按钮

	include_once(ROOT_PATH . '/baoxian/source/function_task.php');
	
	$task = new TaskActive;
	if($task)
	{
		ss_log("将要执行 task_post_policy------------");
		$task->task_post_policy();//定期投保
	}
	
	showmessage("批量投保成功！",'order.php?act=list');
}
elseif ($_REQUEST['act'] == 'list')
{
    ...
    //del yes123 2015-01-31 删除ecshop 自带的查询功能
    ...
    $order_list = order_list();
   	/*add yes123 2015-02-03 载入支付方式 */
    $smarty->assign('pay_list', payment_list());
    $smarty->assign('order_list',   $order_list['orders']);
    $smarty->assign('order_tatol_amount',   $order_list['order_tatol_amount']);
...
//add yes123 2015-01-31 参会报名条件或者分页查询
elseif ($_REQUEST['act'] == 'meeting_query')
{
    /* 检查权限 */
    admin_priv('meeting_list');

    $order_list = meeting_list();
    ...
    $sort_flag  = sort_flag($order_list['filter']);
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
    make_json_result($smarty->fetch('meeting_list.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
...
elseif ($_REQUEST['act'] == 'info')
{
	 ...
	 */
    if($order['receipt_id']){
	  	$sql = "SELECT * FROM " . $ecs->table('receipt') . " WHERE id = ".$order['receipt_id'];
    	$receipt = $db->getRow($sql);
    ...
    /* 其他处理 */
    //modify yes123 2015-01-25 时间格式修正
    ...
    else
    {
        ...
        if($policy_id)
        {
	    	...
	    	if($product_info2['attribute_type'] == 'Y502')
	    	{
		    	...
		    	$smarty->assign('list_product_duty_price', $list_product_duty_price);
		    }
		    $smarty->assign('attribute_type_y502', $product_info2['attribute_type'] );
		    //add end by zhangxi, 20141216
        ...
        */
        ...
        $_REQUEST['order_id'] = $order['order_id'];//start add by wangcya, for bug[193],能够支持多人批量投保

        /*del by wangcya, 20150202
        $policy_temp = get_alluser_policy_list();
        $policy_list = $policy_temp['policys'];
        if($policy_list[0]['attribute_type'] == 'Y502')
        {
        	$policy_list = get_policy_list_by_order_id($order_id);
        }
        */
        
        ss_log("将要根据订单查询到投保单");
        //modify yes123 2015-02-03 
        //$policy_list = get_policy_list_by_order_id($order_id);//都是统一这样处理
        $policy_list = get_policy_list_by_order_id($order['order_id']);//都是统一这样处理
       //echo "<pre>";
       //print_r($policy_list);
        ...
        $smarty->assign('no_show_other', 1);//不显示其他的
        //end add by wangcya, for bug[193],能够支持多人批量投保
                //////////////////end add by wangcya , 20140926//////////////////////////
...
//add yes123 2015-01-31 会议列表
elseif($_REQUEST['act'] == 'meeting_list'){
	    /* 检查权限 */
    admin_priv('meeting_list');
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('meeting_list.htm');
}
/*------------------------------------------------------ */
//-- 发货单列表
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'invoice_list')
{
	...
	///start add by yes123 , 2014-11-12,得到保险公司的列表//////////////
	...
	///end add by yes123 , 20141106/////////////////////////////////////
    /* 检查权限 */
    admin_priv('invoice_list');
    /* 查询 */
 	$result = invoice_list();
    
    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['11_invoice_list']);
    $smarty->assign('full_page',        1);
    //add yes123 2014-11-27  当前页面url,点击受理完毕后用的
  	$_SESSION['current_url'] = $result['current_url'];
  	$smarty->assign('current_url',       $result['current_url']);

    $smarty->assign('invoice_list', $result['invoice_list']);
    $smarty->assign('filter',       $result['filter']);
    $smarty->assign('record_count', $result['invoice_tatol_amount']);
    $smarty->assign('page_count',   $result['page_count']);
    $smarty->assign('see_all',   $result['see_all']);
    /* 显示模板 */
    assign_query_info();
    $smarty->display('invoice_list.htm');
    
    //header("Location: order.php?act=invoice_list");
...
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'invoice_export_list')
{
	//第一是保存id,因为ajax请求只能发送ID。不能导出，第二个请求不家id参数，才导出
	$order_ids = $_REQUEST['order_ids'];
	include_once(ROOT_PATH . 'oop/lib/Excel/PHPExcel.php');
	

	  $sql="SELECT o.goods_amount,o.receipt_assigned," .
				"r.*," .
				"g.goods_name," .
				"p.policy_no,p.applicant_username,p.start_date," .
				"u.user_name FROM " .
				"". $GLOBALS['ecs']->table('order_info') . " o INNER JOIN " .
				"". $GLOBALS['ecs']->table('receipt') . " r ON o.receipt_id=r.id INNER JOIN " .
				" t_insurance_policy p ON o.policy_id=p.policy_id INNER JOIN " .
				"". $GLOBALS['ecs']->table('users') . " u ON u.user_id=o.user_id LEFT JOIN " .
				"". $GLOBALS['ecs']->table('order_goods') . " g ON g.order_id=o.order_id WHERE " .
				" o.receipt_id <> 0 AND o.receipt_id IS NOT NULL AND o.order_id in(".$order_ids.") ORDER BY r.receipt_add_time DESC";
    ...
    $objExcel->setActiveSheetIndex(0)->getColumnDimension('D')->setWidth(10);  
    $objExcel->setActiveSheetIndex(0)->getColumnDimension('E')->setWidth(10);  
    $objExcel->setActiveSheetIndex(0)->getColumnDimension('F')->setWidth(20);  
    $objExcel->setActiveSheetIndex(0)->getColumnDimension('G')->setWidth(20);  
    $objExcel->setActiveSheetIndex(0)->getColumnDimension('H')->setWidth(15);  
    $objExcel->setActiveSheetIndex(0)->getColumnDimension('I')->setWidth(15);  	
	$objExcel->setActiveSheetIndex(0)->getColumnDimension('J')->setWidth(25);  	
	$objExcel->setActiveSheetIndex(0)->getColumnDimension('K')->setWidth(10); 
	
	
	//设置内容居中

	$title = array ('保单号','产品名称','保险起期','投保人','金额','发票抬头','收件人','联系电话','手机号码','邮寄地址','状态');
	
	
	$invoice_array= array();
	$invoice_array[0]=$title;
	if(is_array($invoice_list))    //add
	{  	     
		foreach ($invoice_list as $value) {
				$invoice=array();
				$invoice[0]= $value['policy_no']." ";
				$invoice[1]=$value['goods_name'];
				$invoice[2]=$value['start_date'];
				$invoice[3]=$value['applicant_username'];
				$invoice[4]=$value['goods_amount'];
				$invoice[5]=$value['fp_title'];
				$invoice[6]=$value['username'];
				$invoice[7]=$value['phone']." ";
				$invoice[8]=$value['tel']." ";
				$invoice[9]=$value['address'];
				$invoice[10]=$value['receipt_assigned']==1?"已处理":"未处理";
				$invoice_array[]=$invoice;
		}
	}
	foreach($invoice_array as $k => $v){//每一个$k为一行数据
			$k++;
			foreach($v as $kk => $vs){//每一个$vs是一行中的列
				...
				if($k==1){
				    ...
				    $objFontA1->getColor()->setARGB('FFFFFFFF');  
				    
				    $objStyleA1->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);  
    				$objStyleA1->getFill()->getStartColor()->setARGB("FF365D6A"); 
				}
			}
	}
 ...
 * 根据订单id查询发票信息
 */
elseif ($_REQUEST['act'] == 'findReceiptById')
{
	$sql="SELECT receipt_id,need_receipt FROM ". $ecs->table('order_info') ."WHERE order_id= ".$_REQUEST['order_id'];
	$order_info = $GLOBALS['db']->getRow($sql);
	
	if($order_info['receipt_id']){
		$sql="SELECT * FROM ". $ecs->table('receipt')." WHERE id=".$order_info['receipt_id'];
		$r = $GLOBALS['db']->getRow($sql);
		$r['need_receipt']=$order_info['need_receipt'];
		die(json_encode($r));
	}
	exit;
}
 ...
 * 批量处理发票
 */
elseif ($_REQUEST['act'] == 'batch_invoice_assigned')
{
	include_once (ROOT_PATH . 'includes/class/invoice.class.php');
	$invoice_ids = $_REQUEST['invoice_ids'];
	$r = Invoice::invoiceAssigned($invoice_ids);
...
//-- 搜索、分页
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'invoice_query')
{
    /* 检查权限 */
    admin_priv('invoice_list');
	$result = invoice_list();
    ...
    $smarty->assign('record_count', $result['record_count']);
    $smarty->assign('page_count',   $result['page_count']);
    make_json_result($smarty->fetch('invoice_list.htm'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
}

/**
 * add yes123 2014-11-21 查看发票明细
 */
elseif ($_REQUEST['act'] == 'invoice_detail')
{
    /* 检查权限 */
    admin_priv('invoice_list');
    include_once (ROOT_PATH . 'includes/class/invoice.class.php');
    $invoice = new Invoice;
   	$invoice_detail = $invoice->getInvoiceById($_REQUEST['id']);
	//echo "<pre>";print_r($invoice_detail);
	$smarty->assign('ur_here', "发票详细信息");
    $smarty->assign('invoice_detail', $invoice_detail);
    $smarty->display('invoice_detail.htm');
}
/*------------------------------------------------------ */
//-- 搜索、排序、分页
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('edit_goods' == $step)
    {
        if (isset($_POST['rec_id']))
        {
			...
			$policy_list = $db->getAll($sql);
			
			ss_log("$order_id保单个数：".print_r($policy_list,true));
			
            foreach ($_POST['rec_id'] AS $key => $rec_id)
            {
              $sql = "SELECT goods_number,is_real ".
              ...
              if($goods['is_real']==0)
              {
					...
					{
						$policy_id = $policy_list[0]['policy_id'];
						$sql = " UPDATE t_insurance_policy SET total_premium='$goods_price' WHERE policy_id='$policy_id'";
						ss_log("更新保费:".$sql);
						$db->query($sql);
					...
					{
						sys_msg("此订单有多个保单，无法更新");
						
					}
	                
              	
              }
              else if($goods['goods_number']>=$goods_number)
              {
                /* 修改 */
                $sql = "UPDATE " . $ecs->table('order_goods') .
                        " SET goods_price = '$goods_price', " .
                ...
                if ($order['order_amount'] > 0)
                {
                    ...
                    if ($order['order_amount'] > 0)
                    {
                        ...
                        if (isset($_POST['surplus']) && floatval($_POST['surplus']) >= 0)
                        {
                            ...
                            /* 如果代金券和积分和余额足以支付，把待付款金额改为0，退回部分积分余额 */
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('pay' == $operation)
    {
        ...
        //mod by zhangxi, 20150408
        ss_log("fron admin order, into assign_commision_and_post_policy");
    ...
    elseif ('cancel' == $operation)
    {
    	//判断订单是否为已确认,已付款，并且订单所对应的保单都是投保失败的
    	//1.查询是否有已投的保单
	    $sql = "SELECT policy_id FROM t_insurance_policy WHERE order_id = '$order_id' AND policy_status='insured'";
	    ss_log("1.查询是否有已投的保单：".$sql);
	    $policy = $db->getAll($sql);
	    if($policy)
	    {
	    	ss_log("此订单下有已投保成功的保单，无法取消！".count($policy));
	    	$link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
	    	sys_msg("此订单下有已投保成功的保单，无法取消！", 1, $link);
	    	exit;
	    	
	    }
	    
	    //2.判断是否有已注销的保单
    	$sql = "SELECT policy_id FROM t_insurance_policy WHERE order_id = '$order_id' AND policy_status='canceled'";
    	ss_log("2.判断是否有已注销的保单：".$sql);
	    $policy = $db->getAll($sql);
	    if($policy)
	    {
	    	ss_log("此订单下有已投保成功的保单，无法取消！".count($policy));
	    	$link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
	    	sys_msg("此订单下有已成功注销的保单，无法取消！", 1, $link);
	    	exit;
	    	
	    }
    	
    	//add yes123 2015-02-12 注销订单，要修改的订单信息，佣金
		withdraw_order_fee($order);
...
/**
 * 退回余额、积分、代金券（取消、无效、退货时），把订单使用余额、积分、代金券设为0
...
function return_user_surplus_integral_bonus($order)
{
    ...
    if ($order['user_id'] > 0 && $order['surplus'] > 0)
    {
        ...
        log_account_change($order['user_id'], $surplus, 0, 0, 0, sprintf($GLOBALS['_LANG']['return_order_surplus'], $order['order_sn']),"",$order['order_sn']);
        $GLOBALS['db']->query("UPDATE ". $GLOBALS['ecs']->table('order_info') . " SET `order_amount` = '0' WHERE `order_id` =". $order['order_id']);
        //order_amount应付金额，surplus订单使用的余额，money_paid已付款金额,
    }
    ...
    {
        log_account_change($order['user_id'], 0, 0, 0, $order['integral'], sprintf($GLOBALS['_LANG']['return_order_integral'], $order['order_sn'],$order['order_sn']),"",$order['order_sn']);
    }
...
function invoice_list()
{
    ...
    if ($result === false)
    {
		...
		//$invoice->user_name = trim($_REQUEST['user_name']);//代理人帐号
		//$invoice->real_name = trim($_REQUEST['realname']);//代理人姓名
		...
		$invoice->date_type = trim($_REQUEST['date_type']);
		$invoice->start_time = trim($_REQUEST['start_time']);
		$invoice->end_time = trim($_REQUEST['end_time']);
		$invoice->insurer_code = trim($_REQUEST['insurer_code']);
		$invoice->policy_no = trim($_REQUEST['policy_no']);
		$invoice->applicant_username = trim($_REQUEST['applicant_username']); //投保人
		$invoice->assured_fullname = trim($_REQUEST['assured_fullname']); //投保人
		$invoice->receipt_assigned = trim($_REQUEST['receipt_assigned']); //受理状态
		
		//add yes123 2014-12-16  快递信息	
		$invoice->express_company = trim($_REQUEST['express_company']); //投保人
		$invoice->mail_sn = trim($_REQUEST['mail_sn']); //受理状态
		...
		ss_log(__FUNCTION__." 1.获取角色ID：".$role_sql);
		...
		if($role_id)
		{
			...
			ss_log(__FUNCTION__." 2.获取角色：".$role_name_sql);
			$role = $GLOBALS['db']->getRow($role_name_sql);
			if($role['action_list'])
			{
				$action_list = explode(",",$role['action_list']);
				$var = print_r($action_list , true);
				ss_log(__FUNCTION__." action_list--------------".$var);
				if(in_array('invoice_list',$action_list))
				{
					...
					ss_log(__FUNCTION__." 3.获取insurer_code：".$sql);
					$insurer_code = $GLOBALS['db']->getOne($sql);
					if($insurer_code)//find
					{
						$invoice->insurer_code = trim($insurer_code);
						$see_all = 0;
					}
		...
		$invoice->start = trim(($filter['page'] - 1) * $filter['page_size']);
		$invoice->page_size = trim($filter['page_size']);	
	...
	$filter['record_count'] = $invoice->getInvoiceCount();
	$filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
	
	$invoice_list = $invoice->getInvoiceList();
	
	//add yes123 2014-11-27  当前页面url,点击受理完毕后用的	
	$current_url="order.php?act=invoice_list";
	...
	$arr = array('invoice_list' => $invoice_list,
			'invoice_tatol_amount'=>$filter['record_count'],
			'filter' => $filter, 
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['real_name'])
        {
 			$where .= " AND u.real_name='$filter[real_name]'";
        ...
        { 
            $where .= " AND o.add_time >= '".strtotime($filter['start_time'])."'";
        }
        if ($filter['end_time'])
        {
            $where .= " AND o.add_time <= '".strtotime($filter['end_time'])."'";
        }
	
		//modify yes123 2014-12-05  通过产品名查询
/*        if ($filter['goods_name'])
        {
          	$where.=" AND ip.goods_name LIKE '%".$filter['goods_name']."%'";
        }*/

        //start add by wangcya, 20150119
        if ($filter['platform_id'])
        {
        	$where.=" AND o.platform_id LIKE '%".$filter['platform_id']."%'";
        }
        ...
        switch($filter['composite_status'])
        {
            ...
            case OS_SHIPPED_PART :
                ...
                break;
           case CS_PAYFINISHED :// 已确认、已付款
                if ($filter['composite_status'] != -1)
                {
                    $where .= " AND o.order_status  =1 AND o.pay_status=2 ";
                }
                break;
           //added by zhangxi, 20150214, 已确认，已付款，投保失败的异常情况
           case CS_PAYFINISHED_INSURE_FAILED :
           		if ($filter['composite_status'] != -1)
                {
                    $where .= " AND o.order_status  =1 AND o.pay_status=2 AND ip.policy_status='payed'";
                }
           		break;
            default:
                if ($filter['composite_status'] != -1)
	   ...
	   $record_count_sql ="SELECT SUM(o.goods_amount) AS total, COUNT(*) AS count FROM " .$GLOBALS['ecs']->table('order_info')." AS o " .
				  " INNER JOIN " .$GLOBALS['ecs']->table('users')." AS u ON u.user_id = o.user_id " .$where;
				
	   $value_count = $GLOBALS['db']->getRow($record_count_sql);
       
       $order_tatol_amount = $value_count['total'];
       $filter['record_count'] = $value_count['count'];
	   $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
    ...
    foreach ($row AS $key => $value)
    {
    	...
    	//start add by wangcya, 20150325,把那些已经支付但是没投保成功的突出显示出来
    	if($value['policy_num'] ==  $value['insured_policy_num'])
    	{//如果订单下面的保单个数和已经投保的个数相符，则表示完成
    		$row[$key]['policy_status'] = "完成";
    	}
    	else
    	{

			if($value['order_status']==1 && $value['pay_status']==2) //订单状态是已支付，已确认
			{
				$row[$key]['policy_status_red'] = 1;//突出显示
				$row[$key]['policy_status'] = "未完成";
			}
			else
			{
				$row[$key]['policy_status'] = "";//没投保的就不填写
			}
		
    		
    	}
    	//end add by wangcya, 20150325,把那些已经支付但是没投保成功的突出显示出来
    	
    	$row[$key]['formated_order_amount'] = price_format($value['order_amount']);
        $row[$key]['formated_money_paid'] = price_format($value['money_paid']);
        $row[$key]['formated_total_fee'] = price_format($value['total_fee']);
        //$row[$key]['short_order_time'] = local_date('m-d H:i', $value['add_time']);
        $row[$key]['short_order_time'] = date('Y-m-d H:i', $value['add_time']);
        ...
        //add yes123 2015-05-13 计算实际支付方式 actual_pay_status
        if($value['pay_status']==2)
        {
	        $row[$key]['actual_pay_status']=$value['pay_name'];
        }
        if($value['pay_name']!='余额支付')
        {
        	if($value['surplus']>0)
        	{
        		$temp_value = $value['goods_amount']-$value['surplus'];
        		if($temp_value==0)
        		{
        			$row[$key]['actual_pay_status']='余额支付';
        		}
        		elseif($temp_value>0)
        		{
        			$row[$key]['actual_pay_status']='混合支付';
        		}
        ...
        //查询商品
        $sql = " SELECT * FROM bx_order_goods WHERE order_id=$value[order_id]";
        $goods_list = $GLOBALS['db']->getAll($sql);
        $row[$key]['goods_list'] = $goods_list;
        
    }
    
    $arr = array('orders' => $row,
...
function meeting_list(){
    ...
    if ($result === false)
    {
        ...
        if ($filter['goods_name'])
        {
          	$where.=" AND ci.product_name LIKE '%".$filter['goods_name']."%'";
        }
		...
		$record_count_sql ="SELECT count(o.order_id)  FROM " .$GLOBALS['ecs']->table('order_info'). " AS o " .
   				" INNER JOIN t_cart_insure AS ci ON o.cart_insure_id=ci.rec_id  " .
   				" INNER JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON o.user_id=u.user_id ".$where;
   		ss_log('meeting_list record_count_sql:'.$record_count_sql);
 		$filter['record_count'] = $GLOBALS['db']->getOne($record_count_sql);//add by wangcya, for bug[193],能够支持多人批量投保
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

        /* 查询 */
   		$sql="SELECT o.*,ci.*,u.user_name,u.real_name FROM " .$GLOBALS['ecs']->table('order_info'). " AS o " .
   				" INNER JOIN t_cart_insure AS ci ON o.cart_insure_id=ci.rec_id  " .
   				" INNER JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON o.user_id=u.user_id ".$where.
   				" ORDER BY $filter[sort_by] $filter[sort_order] ".
        		" LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
    	ss_log('meeting_list sql:'.$sql);
        set_filter($filter, $sql);
    }
    else
    {
    ...
    $order_tatol_amount=0;
    //统计订单金额总数2014-09-27 yes123
   	$sql ="SELECT SUM(o.goods_amount)  FROM " .$GLOBALS['ecs']->table('order_info'). " AS o " .
   				" INNER JOIN t_cart_insure AS ci ON o.cart_insure_id=ci.rec_id  " .
   				" INNER JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON o.user_id=u.user_id ".$where;
...
function delivery_order_info($delivery_id, $delivery_sn = '')
{
    ...
    if ($delivery)
    {
        ...
        /* 格式化时间字段 */
        $delivery['formated_add_time'] =  date("Y-m-d H:i:s", $delivery['add_time']);
...
function back_order_info($back_id)
{
    ...
    if ($back)
    {
        ...
        $back['formated_add_time']       = date("Y-m-d H:i:s", $back['add_time']);
